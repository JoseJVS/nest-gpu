...
params = {
    ...
    "record_spikes": (
        args.record_spikes != 0 and args.opt < 3
    ...
    "raster_plot": (
        args.raster_plot != 0
    ),  # when record_spikes=True, depicts a raster plot
...
def build_network():
    ...
    ngpu.ConnectDistributedFixedIndegree(
        host_list, E_pops_list, host_list, neurons_list, CE, hg, syn_dict_ex
    )
    ngpu.ConnectDistributedFixedIndegree(
        host_list, I_pops_list, host_list, neurons_list, CI, hg, syn_dict_in
    )
