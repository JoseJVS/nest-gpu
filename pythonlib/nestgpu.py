...
class ConnectionList(object):
    def __init__(self, conn_list):
        if (type(conn_list) != list) & (type(conn_list) != tuple):
            raise ValueError(
                "ConnectionList object can be initialized only"
...
def Create(model_name, n_node=1, n_ports=1, status_dict=None):
    ...
    c_model_name = ctypes.create_string_buffer(
        to_byte_str(model_name), len(model_name) + 1
    )
...
def CreateRecord(file_name, var_name_list, i_node_list, i_port_list):
    ...
    c_file_name = ctypes.create_string_buffer(
        to_byte_str(file_name), len(file_name) + 1
    )
    array_int_type = ctypes.c_int * n_node
    array_char_pt_type = c_char_p * n_node
    c_var_name_list = []
    for i in range(n_node):
        c_var_name = ctypes.create_string_buffer(
            to_byte_str(var_name_list[i]), len(var_name_list[i]) + 1
        )
...
def SetNeuronScalParam(i_node, n_node, param_name, val):
    ...
    c_param_name = ctypes.create_string_buffer(
        to_byte_str(param_name), len(param_name) + 1
    )
    ret = NESTGPU_SetNeuronScalParam(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_param_name, ctypes.c_float(val)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def SetNeuronPtScalParam(nodes, param_name, val):
    ...
    ret = NESTGPU_SetNeuronPtScalParam(
        node_pt, ctypes.c_int(n_node), c_param_name, ctypes.c_float(val)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def SetNeuronIntVar(i_node, n_node, var_name, val):
    ...
    ret = NESTGPU_SetNeuronIntVar(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_var_name, ctypes.c_int(val)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def SetNeuronScalVar(i_node, n_node, var_name, val):
    ...
    c_var_name = ctypes.create_string_buffer(to_byte_str(var_name), len(var_name) + 1)
    ret = NESTGPU_SetNeuronScalVar(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_var_name, ctypes.c_float(val)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def SetNeuronPtIntVar(nodes, var_name, val):
    ...
    ret = NESTGPU_SetNeuronPtIntVar(
        node_pt, ctypes.c_int(n_node), c_var_name, ctypes.c_int(val)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def SetNeuronPtScalVar(nodes, var_name, val):
    ...
    ret = NESTGPU_SetNeuronPtScalVar(
        node_pt, ctypes.c_int(n_node), c_var_name, ctypes.c_float(val)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_SetNeuronScalParamDistr.argtypes = (ctypes.c_int, ctypes.c_int, c_char_p)
...
def SetNeuronScalParamDistr(i_node, n_node, param_name):
    ...
    ret = NESTGPU_SetNeuronScalParamDistr(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_param_name
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_SetNeuronScalVarDistr.argtypes = (ctypes.c_int, ctypes.c_int, c_char_p)
...
def SetNeuronScalVarDistr(i_node, n_node, var_name):
    ...
    c_var_name = ctypes.create_string_buffer(to_byte_str(var_name), len(var_name) + 1)
    ret = NESTGPU_SetNeuronScalVarDistr(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_var_name
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_SetNeuronPortParamDistr.argtypes = (ctypes.c_int, ctypes.c_int, c_char_p)
...
def SetNeuronPortParamDistr(i_node, n_node, param_name):
    ...
    ret = NESTGPU_SetNeuronPortParamDistr(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_param_name
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
...
NESTGPU_SetNeuronPortVarDistr.argtypes = (ctypes.c_int, ctypes.c_int, c_char_p)
...
def SetNeuronPortVarDistr(i_node, n_node, var_name):
    ...
    c_var_name = ctypes.create_string_buffer(to_byte_str(var_name), len(var_name) + 1)
    ret = NESTGPU_SetNeuronPortVarDistr(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_var_name
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
...
NESTGPU_SetNeuronPtScalParamDistr.argtypes = (ctypes.c_void_p, ctypes.c_int, c_char_p)
...
def SetNeuronPtScalParamDistr(nodes, param_name):
    ...
    node_arr = (ctypes.c_int * len(nodes))(*nodes)
    node_pt = ctypes.cast(node_arr, ctypes.c_void_p)
    ret = NESTGPU_SetNeuronPtScalParamDistr(node_pt, ctypes.c_int(n_node), c_param_name)
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_SetNeuronPtScalVarDistr.argtypes = (ctypes.c_void_p, ctypes.c_int, c_char_p)
...
def SetNeuronPtScalVarDistr(nodes, var_name):
    ...
    node_arr = (ctypes.c_int * len(nodes))(*nodes)
    node_pt = ctypes.cast(node_arr, ctypes.c_void_p)
    ret = NESTGPU_SetNeuronPtScalVarDistr(node_pt, ctypes.c_int(n_node), c_var_name)
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_SetNeuronPtPortParamDistr.argtypes = (ctypes.c_void_p, ctypes.c_int, c_char_p)
...
def SetNeuronPtPortParamDistr(nodes, param_name):
    ...
    node_arr = (ctypes.c_int * len(nodes))(*nodes)
    node_pt = ctypes.cast(node_arr, ctypes.c_void_p)
    ret = NESTGPU_SetNeuronPtPortParamDistr(node_pt, ctypes.c_int(n_node), c_param_name)
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_SetNeuronPtPortVarDistr.argtypes = (ctypes.c_void_p, ctypes.c_int, c_char_p)
...
def SetNeuronPtPortVarDistr(nodes, var_name):
    ...
    node_arr = (ctypes.c_int * len(nodes))(*nodes)
    node_pt = ctypes.cast(node_arr, ctypes.c_void_p)
    ret = NESTGPU_SetNeuronPtPortVarDistr(node_pt, ctypes.c_int(n_node), c_var_name)
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def SetDistributionIntParam(param_name, val):
    ...
    ret = NESTGPU_SetDistributionIntParam(c_param_name, ctypes.c_int(val))
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def SetDistributionScalParam(param_name, val):
    ...
    ret = NESTGPU_SetDistributionScalParam(c_param_name, ctypes.c_float(val))
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_SetDistributionVectParam.argtypes = (c_char_p, ctypes.c_float, ctypes.c_int)
...
def SetDistributionVectParam(param_name, val, i):
    ...
    ret = NESTGPU_SetDistributionVectParam(
        c_param_name, ctypes.c_float(val), ctypes.c_int(i)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_SetDistributionFloatPtParam = _nestgpu.NESTGPU_SetDistributionFloatPtParam
...
def GetNeuronParam(i_node, n_node, param_name):
    ...
    data_pt = NESTGPU_GetNeuronParam(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_param_name
    )
...
def GetNeuronIntVar(i_node, n_node, var_name):
    ...
    c_var_name = ctypes.create_string_buffer(to_byte_str(var_name), len(var_name) + 1)
    data_pt = NESTGPU_GetNeuronIntVar(
        ctypes.c_int(i_node), ctypes.c_int(n_node), c_var_name
    )
...
def GetNeuronStatus(nodes, var_name):
    ...
    if type(nodes) == NodeSeq:
        if IsNeuronScalParam(nodes.i0, var_name) | IsNeuronPortParam(
            nodes.i0, var_name
        ):
        ...
        if IsNeuronScalParam(nodes[0], var_name) | IsNeuronPortParam(
            nodes[0], var_name
        ):
...
def GetIntVarNames(i_node):
    ...
    var_name_pp = ctypes.cast(
        NESTGPU_GetIntVarNames(ctypes.c_int(i_node)), ctypes.POINTER(c_char_p)
    )
    var_name_list = []
...
def GetScalVarNames(i_node):
    ...
    var_name_pp = ctypes.cast(
        NESTGPU_GetScalVarNames(ctypes.c_int(i_node)), ctypes.POINTER(c_char_p)
    )
    var_name_list = []
    for i in range(n_var):
        var_name_p = var_name_pp[i]
...
def GetPortVarNames(i_node):
    ...
    var_name_pp = ctypes.cast(
        NESTGPU_GetPortVarNames(ctypes.c_int(i_node)), ctypes.POINTER(c_char_p)
    )
    var_name_list = []
    for i in range(n_var):
        var_name_p = var_name_pp[i]
...
def GetScalParamNames(i_node):
    ...
    param_name_pp = ctypes.cast(
        NESTGPU_GetScalParamNames(ctypes.c_int(i_node)), ctypes.POINTER(c_char_p)
    )
    param_name_list = []
...
def GetPortParamNames(i_node):
    ...
    param_name_pp = ctypes.cast(
        NESTGPU_GetPortParamNames(ctypes.c_int(i_node)), ctypes.POINTER(c_char_p)
    )
    param_name_list = []
    for i in range(n_param):
        param_name_p = param_name_pp[i]
...
def GetArrayParamNames(i_node):
    ...
    param_name_pp = ctypes.cast(
        NESTGPU_GetArrayParamNames(ctypes.c_int(i_node)), ctypes.POINTER(c_char_p)
    )
    param_name_list = []
    for i in range(n_param):
        param_name_p = param_name_pp[i]
...
def GetGroupParamNames(i_node):
    ...
    param_name_pp = ctypes.cast(
        NESTGPU_GetGroupParamNames(ctypes.c_int(i_node)), ctypes.POINTER(c_char_p)
    )
    param_name_list = []
    for i in range(n_param):
        param_name_p = param_name_pp[i]
...
def GetArrayVarNames(i_node):
    ...
    var_name_pp = ctypes.cast(
        NESTGPU_GetArrayVarNames(ctypes.c_int(i_node)), ctypes.POINTER(c_char_p)
    )
    var_name_list = []
    for i in range(n_var):
        var_name_p = var_name_pp[i]
    ...
    if type(val) == dict:
        if (
            (
                type(nodes) == NodeSeq
                and (
                    IsNeuronScalParam(nodes.i0, var_name)
                    ...
                    if IsDistributionFloatParam(dict_param_name):
                        if (
                            ...
                            for i, value in enumerate(pval):
                                SetDistributionVectParam(dict_param_name, value, i)
                    else:
        ...
        elif IsNeuronPortParam(nodes.i0, var_name) | IsNeuronArrayParam(
            nodes.i0, var_name
        ):
        ...
        elif IsNeuronPortParam(nodes[0], var_name) | IsNeuronArrayParam(
            nodes[0], var_name
        ):
...
def SetConnectionStatus(conn, param_name, val):
    ...
    if (type(conn) != list) and (type(conn) != tuple):
        raise ValueError(
            "SetConnectionStatus argument 1 type must be "
        ...
        for dict_param_name in val:
            ...
            if dict_param_name == "array":
                ...
                if not IsConnectionFloatParam(param_name):
                    raise ValueError(
                        "Only float connection parameters can be"
...
def FakeConnectMpiInit(n_hosts, this_host):
    ...
    ret = NESTGPU_FakeConnectMpiInit(ctypes.c_int(n_hosts), ctypes.c_int(this_host))
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def RandomNormal(n, mean, stddev):
    ...
    ret = NESTGPU_RandomNormal(
        ctypes.c_size_t(n), ctypes.c_float(mean), ctypes.c_float(stddev)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
NESTGPU_RandomLognormalClipped.argtypes = (
    ctypes.c_size_t,
    ctypes.c_float,
    ctypes.c_float,
    ctypes.c_float,
...
def RandomLognormalClipped(n, mean, stddev, vmin, vmax, vstep):
    ...
    ret = NESTGPU_RandomLognormalClipped(
        ctypes.c_size_t(n),
        ctypes.c_float(mean),
...
def Connect(source, target, conn_dict, syn_dict):
            ...
            if type(fpar) == dict:
                for dict_param_name in fpar:
                    ...
                    if dict_param_name == "array":
                        ...
                        if not SynSpecIsFloatPtParam(arr_param_name):
                            raise ValueError(
                                "Synapse parameter cannot be set by arrays"
                            )
                        array_pt = ctypes.cast(arr, ctypes.c_void_p)
                        SetSynSpecFloatPtParam(arr_param_name, array_pt)
                    elif dict_param_name == "distribution":
                        ...
                        if not SynSpecIsIntParam(distr_param_name):
                            raise ValueError(
                                "Synapse parameter cannot be set by distributions"
                            )
    ...
    if (type(source) == NodeSeq) & (type(target) == NodeSeq):
        ...
        if (type(source) == NodeSeq) & (type(target) != NodeSeq):
            ret = NESTGPU_ConnectSeqGroup(
                source.i0, source.n, target_arr_pt, len(target)
            )
        elif (type(source) != NodeSeq) & (type(target) == NodeSeq):
            ret = NESTGPU_ConnectGroupSeq(
                source_arr_pt, len(source), target.i0, target.n
            )
        else:
            ret = NESTGPU_ConnectGroupGroup(
                source_arr_pt, len(source), target_arr_pt, len(target)
            )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    gc.enable()
...
def RemoteConnect(
    i_source_host, source, i_target_host, target, conn_dict, syn_dict, host_group=-1
):
    ...
    for param_name in syn_dict:
        if SynSpecIsIntParam(param_name):
            ...
            raise ValueError("Unknown synapse parameter")
    if (type(source) == NodeSeq) & (type(target) == NodeSeq):
        ret = NESTGPU_RemoteConnectSeqSeq(
            i_source_host,
            source.i0,
            source.n,
            i_target_host,
...
def SetStatus(gen_object, params, val=None):
    ...
    if (
        (type(gen_object) != list)
        and (type(gen_object) != tuple)
        and (type(gen_object) != NodeSeq)
        and (type(gen_object) != RemoteNodeSeq)
        and (type(gen_object) != ConnectionList)
        and (type(gen_object) != SynGroup)
    ):
    ...
    if val != None:
        ...
        if len(params) != len(gen_object):
            raise ValueError(
                "List should have the same size as the first argument of SetStatus"
            )
        for param_dict in params:
            ...
            for param_name in param_dict:
                if type(gen_object) == ConnectionList:
                    SetConnectionStatus(gen_object, param_name, param_dict[param_name])
                else:
                    SetNeuronStatus(gen_object, param_name, param_dict[param_name])
    else:
...
def GetConnectionStatus(conn):
    ...
    if (type(conn) != list) and (type(conn) != tuple):
        raise ValueError(
            "GetConnectionStatus argument type must be "
            "ConnectionList, int, list or tuple"
        )
    n_conn = len(conn)
    ...
    NESTGPU_GetConnectionStatus(
        conn_arr, n_conn, i_source, i_target, i_port, i_syn_group, delay, weight
    )
    status_list = []
...
NESTGPU_GetConnectionFloatParam.argtypes = (
    c_int64_p,
    ctypes.c_int64,
    c_float_p,
    c_char_p,
...
def GetConnectionFloatParam(conn, param_name):
    ...
    if (type(conn) != list) and (type(conn) != tuple):
        raise ValueError(
            "GetConnectionFloatParam argument 1 type must be "
            "ConnectionList, int, list or tuple"
        )
...
NESTGPU_GetConnectionIntParam.argtypes = (c_int64_p, ctypes.c_int64, c_int_p, c_char_p)
...
def GetConnectionIntParam(conn, param_name):
    ...
    if (type(conn) != list) and (type(conn) != tuple):
        raise ValueError(
            "GetConnectionIntParam argument 1 type must be "
            "ConnectionList, int, list or tuple"
        )
...
NESTGPU_SetConnectionFloatParamDistr = _nestgpu.NESTGPU_SetConnectionFloatParamDistr
NESTGPU_SetConnectionFloatParamDistr.argtypes = (c_int64_p, ctypes.c_int64, c_char_p)
...
def SetConnectionFloatParamDistr(conn, param_name):
    ...
    if (type(conn) != list) and (type(conn) != tuple):
        raise ValueError(
            "SetConnectionFloatParamDistr argument 1 type must be"
            " ConnectionList, int, list or tuple"
        )
...
NESTGPU_SetConnectionFloatParam.argtypes = (
    c_int64_p,
    ctypes.c_int64,
    ctypes.c_float,
    c_char_p,
...
def SetConnectionFloatParam(conn, param_name, val):
    ...
    if (type(conn) != list) and (type(conn) != tuple):
        raise ValueError(
            "SetConnectionFloatParam argument 1 type must be "
            "ConnectionList, int, list or tuple"
        )
    ...
    ret = NESTGPU_SetConnectionFloatParam(
        conn_arr, n_conn, ctypes.c_float(val), c_param_name
    )
...
NESTGPU_SetConnectionIntParamArr.argtypes = (
    c_int64_p,
    ctypes.c_int64,
    c_int_p,
    c_char_p,
...
def SetConnectionIntParamArr(conn, param_name, param_arr):
    ...
    if (type(conn) != list) and (type(conn) != tuple):
        raise ValueError(
            "SetConnectionIntParamArr argument 1 type must be "
            "ConnectionList, int, list or tuple"
        )
    ...
    ret = NESTGPU_SetConnectionIntParamArr(
        conn_arr, n_conn, c_int_p(param_arr), c_param_name
    )
...
NESTGPU_SetConnectionIntParam.argtypes = (
    c_int64_p,
    ctypes.c_int64,
    ctypes.c_int,
    c_char_p,
...
def SetConnectionIntParam(conn, param_name, val):
    ...
    if (type(conn) != list) and (type(conn) != tuple):
        raise ValueError(
            "SetConnectionIntParam argument 1 type must be "
            "ConnectionList, int, list or tuple"
        )
    ...
    ret = NESTGPU_SetConnectionIntParam(conn_arr, n_conn, val, c_param_name)
...
def SetSynGroupParam(syn_group, param_name, val):
    ...
    ret = NESTGPU_SetSynGroupParam(
        ctypes.c_int(i_syn_group), c_param_name, ctypes.c_float(val)
    )
...
def GetRecSpikeTimes(nodes):
    ...
    spike_time_list = []
    ret1 = NESTGPU_GetRecSpikeTimes(
        ctypes.c_int(nodes.i0), ctypes.c_int(nodes.n), n_spike_times_pt, spike_times_pt
    )
...
def SetNeuronGroupParam(nodes, param_name, val):
    ...
    ret = NESTGPU_SetNeuronGroupParam(
        ctypes.c_int(nodes.i0), ctypes.c_int(nodes.n), c_param_name, ctypes.c_float(val)
    )
    if GetErrorCode() != 0:
        raise ValueError(GetErrorMessage())
    return ret
...
def RemoteCreate(i_host, model_name, n_node=1, n_ports=1, status_dict=None):
    ...
    i_node = NESTGPU_RemoteCreate(
        ctypes.c_int(i_host), c_model_name, ctypes.c_int(n_node), ctypes.c_int(n_ports)
    )
...
NESTGPU_ConnectDistributedFixedIndegreeSeqSeq = (
    _nestgpu.NESTGPU_ConnectDistributedFixedIndegreeSeqSeq
)
NESTGPU_ConnectDistributedFixedIndegreeSeqSeq.argtypes = (
    ctypes.c_void_p,
    ctypes.c_int,
    ctypes.c_void_p,
    ctypes.c_void_p,
...
NESTGPU_ConnectDistributedFixedIndegreeGroupSeq = (
    _nestgpu.NESTGPU_ConnectDistributedFixedIndegreeGroupSeq
)
NESTGPU_ConnectDistributedFixedIndegreeGroupSeq.argtypes = (
    ctypes.c_void_p,
    ctypes.c_int,
    ctypes.c_void_p,
    ctypes.c_void_p,
...
NESTGPU_ConnectDistributedFixedIndegreeSeqGroup = (
    _nestgpu.NESTGPU_ConnectDistributedFixedIndegreeSeqGroup
)
NESTGPU_ConnectDistributedFixedIndegreeSeqGroup.argtypes = (
    ctypes.c_void_p,
    ctypes.c_int,
    ctypes.c_void_p,
    ctypes.c_void_p,
...
NESTGPU_ConnectDistributedFixedIndegreeGroupGroup = (
    _nestgpu.NESTGPU_ConnectDistributedFixedIndegreeGroupGroup
)
NESTGPU_ConnectDistributedFixedIndegreeGroupGroup.argtypes = (
    ctypes.c_void_p,
    ctypes.c_int,
    ctypes.c_void_p,
    ctypes.c_void_p,
...
def ConnectDistributedFixedIndegree(
    ...
    if (type(source_group_list[0]) == NodeSeq) and (
        type(target_group_list[0]) == NodeSeq
    ):
        ret = NESTGPU_ConnectDistributedFixedIndegreeSeqSeq(
            source_host_arr_pt,
            len(source_host_list),
            source_i0_arr_pt,
            source_n_arr_pt,
            target_host_arr_pt,
            len(target_host_list),
            target_i0_arr_pt,
            target_n_arr_pt,
            indegree,
            host_group,
    ...
    elif (type(source_group_list[0]) == NodeSeq) and (
        type(target_group_list[0]) != NodeSeq
    ):
