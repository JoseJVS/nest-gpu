...
public:
  ...
  virtual void reseed( std::initializer_list< std::uint32_t > ) = 0;
  virtual void shuffle( std::vector< int >& vec ) = 0;
  virtual void shuffle( std::vector< long >& vec ) = 0;
};
...
public:
  ...
  inline void
  reseed(std::initializer_list< std::uint32_t > seed ) override
  {
    randutils::seed_seq_fe128 sseq( seed );
    rng_.seed( sseq );
  }
  ...
  inline void
  shuffle( std::vector< int >& vec ) override
  {
    std::shuffle( vec.begin(), vec.end(), rng_ );
  }

  inline void
  shuffle( std::vector< long >& vec ) override
  {
    std::shuffle( vec.begin(), vec.end(), rng_ );
  }
