...
__global__ void izhikevich_Update(int n_node, int i_node_0, float *var_arr,
                                  float *param_arr, int n_var, int n_param,
                                  float V_th, float a, float b, float c,
                                  float d, int n_refractory_steps, float h) {
  ...
  if (i_neuron < n_node) {
    ...
    if (refractory_step > 0.0) {
      ...
      V_m += h * (0.04 * v_old * v_old + 5.0 * v_old + 140.0 - u_old + I_e) +
             I_syn;
      u += h * a * (b * v_old - u_old);
    }
    I_syn = 0;
...
int izhikevich::Init(int i_node_0, int n_node, int /*n_port*/, int i_group) {
...
int izhikevich::Update(long long it, double t1) {
  ...
  izhikevich_Update<<<(n_node_ + 1023) / 1024, 1024>>>(
      n_node_, i_node_0_, var_arr_, param_arr_, n_var_, n_param_, V_th_, a_, b_,
      c_, d_, n_refractory_steps, h);
