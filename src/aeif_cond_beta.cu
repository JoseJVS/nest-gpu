...
__device__ void NodeCalibrate(int n_var, int n_param, double x, float *y,
  ...
  if (denom1 != 0) {
    // peak time
    float t_p =
        tau_decay_ex * tau_rise_ex * log(tau_decay_ex / tau_rise_ex) / denom1;
  ...
  if (denom1 != 0) {
    // peak time
    float t_p =
        tau_decay_in * tau_rise_in * log(tau_decay_in / tau_rise_in) / denom1;
    // another denominator is computed here to check that it is != 0
...
int aeif_cond_beta::Init(int i_node_0, int n_node, int n_port, int i_group) {
