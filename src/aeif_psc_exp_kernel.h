...
enum ScalVarIndexes { i_I_syn_ex = 0, i_I_syn_in, i_V_m, i_w, N_SCAL_VAR };
...
const std::string aeif_psc_exp_scal_var_name[N_SCAL_VAR] = {
    "I_syn_ex", "I_syn_in", "V_m", "w"};

const std::string aeif_psc_exp_scal_param_name[N_SCAL_PARAM] = {
    "tau_syn_ex", "tau_syn_in", "V_th",    "Delta_T", "g_L",
    "E_L",        "C_m",        "a",       "b",       "tau_w",
    "I_e",        "V_peak",     "V_reset", "t_ref",   "refractory_step",
    "den_delay"};

const std::string aeif_psc_exp_group_param_name[N_GROUP_PARAM] = {"h_min_rel",
                                                                  "h0_rel"};
...
__device__ void ExternalUpdate(double x, float *y, float *param,
                               bool end_time_step,
                               aeif_psc_exp_rk5 data_struct) {
  aeif_psc_exp_ns::ExternalUpdate<NVAR, NPARAM>(x, y, param, end_time_step,
                                                data_struct);
}
