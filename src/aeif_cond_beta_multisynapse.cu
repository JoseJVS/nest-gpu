...
__device__ void NodeCalibrate(int n_var, int n_param, double x, float *y,
  ...
  for (int i = 0; i < n_port; i++) {
    ...
    if (denom1 != 0) {
      // peak time
      float t_p =
          tau_decay(i) * tau_rise(i) * log(tau_decay(i) / tau_rise(i)) / denom1;
...
__device__ void NodeInit(int n_var, int n_param, double x, float *y,
                         float *param,
                         aeif_cond_beta_multisynapse_rk5 data_struct) {
  aeif_cond_beta_multisynapse_ns::NodeInit(n_var, n_param, x, y, param,
                                           data_struct);
}
...
{
  aeif_cond_beta_multisynapse_ns::NodeCalibrate(n_var, n_param, x, y, param,
                                                data_struct);
}
...
int aeif_cond_beta_multisynapse::Calibrate(double time_min,
