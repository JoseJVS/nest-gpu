...
__device__ void PushSpike(int i_spike_buffer, float mul) {
  if (input_spike_buffer_ns::algo_ == INPUT_SPIKE_BUFFER_ALGO) {
    ...
    if (i_conn >= 0) {
      ...
      // printf("PushSpike i_spike_buffer: %d\ti_conn: %ld\tpos:
      // %d\tspike_first_connection[pos]: %ld\n", i_spike_buffer, i_conn, pos,
      // spike_first_connection_[pos]);
    }
  }
  ...
  if (i_spike_buffer < n_local_nodes) {
    ...
    if (max_n_rec_spike_times != 0) {
      ...
      if (n_rec_spike_times >= max_n_rec_spike_times - 1) {
        printf("Maximum number of recorded spike times %d exceeded"
               " for spike buffer %d\n",
               max_n_rec_spike_times, i_spike_buffer);
      } else { // record spike time
        NodeGroupArray[i_group]
            .rec_spike_times_[i_node_rel * max_n_rec_spike_times +
                              n_rec_spike_times] = NESTGPUTime;
        NodeGroupArray[i_group].n_rec_spike_times_[i_node_rel]++;
      }
    }
  }
  ...
  if (input_spike_buffer_ns::algo_ != INPUT_SPIKE_BUFFER_ALGO) {
    ...
    uint conn_group_num =
        ConnGroupIdx0[i_spike_buffer + 1] - ConnGroupIdx0[i_spike_buffer];
...
__global__ void SpikeBufferUpdate() {
  ...
  if (i_spike_buffer < n_local_nodes) {
    ...
    if (den_delay_arr != nullptr) {
      ...
      // node of the group. The other are separate by steps = n_param
      den_delay_idx =
          (int)round(den_delay_arr[i_neuron * n_param] / NESTGPUTimeResolution);
  ...
  for (int is = 0; is < Ns; is++) {
    ...
    if (i_conn < conn_group_num) {
      ...
      if (spike_time_idx + 1 == ConnGroupDelay[ig]) {
        ...
        // deliver spike
        int64_t conn_group_n_conn =
            ConnGroupIConn0[ig + 1] - conn_group_i_conn0;
...
int spikeBufferInit(uint n_local_nodes, uint n_image_nodes,
  ...
  uint n_spike_buffers = (spike_buffer_algo == INPUT_SPIKE_BUFFER_ALGO)
                             ? n_local_nodes
                             : (n_local_nodes + n_image_nodes);

  h_NSpikeBuffer = n_spike_buffers;

  CUDAMALLOCCTRL("&d_LastSpikeTimeIdx", &d_LastSpikeTimeIdx,
                 n_spike_buffers * sizeof(long long));
  CUDAMALLOCCTRL("&d_LastSpikeMul", &d_LastSpikeMul,
                 n_spike_buffers * sizeof(float));
  CUDAMALLOCCTRL("&d_LastRevSpikeTimeIdx", &d_LastRevSpikeTimeIdx,
                 n_spike_buffers * sizeof(long long));
  if (spike_buffer_algo == INPUT_SPIKE_BUFFER_ALGO) {
    ...
    CUDAMALLOCCTRL("&d_SpikeBufferSize", &d_SpikeBufferSize,
                   n_spike_buffers * sizeof(int));
    CUDAMALLOCCTRL("&d_SpikeBufferIdx0", &d_SpikeBufferIdx0,
                   n_spike_buffers * sizeof(int));
    ...
    CUDAMALLOCCTRL("&d_SpikeBufferConnIdx", &d_SpikeBufferConnIdx,
                   n_spike_buffers * max_spike_buffer_size * sizeof(int));
    CUDAMALLOCCTRL("&d_SpikeBufferMul", &d_SpikeBufferMul,
                   n_spike_buffers * max_spike_buffer_size * sizeof(float));
    gpuErrchk(
        cudaMemsetAsync(d_SpikeBufferSize, 0, n_spike_buffers * sizeof(int)));
    gpuErrchk(
        cudaMemsetAsync(d_SpikeBufferIdx0, 0, n_spike_buffers * sizeof(int)));
  }

  DeviceSpikeBufferInit<<<1, 1>>>(n_spike_buffers, max_spike_buffer_size,
                                  d_LastSpikeTimeIdx, d_LastSpikeMul,
                                  d_SpikeBufferSize, d_SpikeBufferIdx0,
                                  d_SpikeBufferTimeIdx, d_SpikeBufferConnIdx,
                                  d_SpikeBufferMul, d_LastRevSpikeTimeIdx);
  gpuErrchk(cudaPeekAtLastError());

  InitLastSpikeTimeIdx<<<(n_spike_buffers + 1023) / 1024, 1024>>>(
      n_spike_buffers, LAST_SPIKE_TIME_GUARD);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaMemsetAsync(d_LastSpikeMul, 0,
