...
const std::string parrot_neuron_scal_var_name[N_PARROT_NEURON_SCAL_VAR] = {
    "input_spike_mul", "dummy_input", "V"};

__global__ void parrot_neuron_UpdateKernel(int i_node_0, int n_node,
  ...
  if (irel_node < n_node) {
    float *input_spike_mul_pt =
        var_arr + irel_node * n_var + i_parrot_neuron_input_spike_mul;
    float *V_pt = var_arr + irel_node * n_var + i_parrot_neuron_V;
    float *hold_spike_mul_pt =
        param_arr + irel_node * n_param + i_parrot_neuron_hold_spike_mul;
...
int parrot_neuron::Init(int i_node_0, int n_node, int /*n_port*/, int i_group) {
  ...
  gpuErrchk(cudaMemcpy(port_weight_arr_, input_weight, 2 * sizeof(float),
