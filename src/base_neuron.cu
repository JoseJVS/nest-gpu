...
int BaseNeuron::Init(int i_node_0, int n_node, int n_port, int i_group) {
...
int BaseNeuron::AllocParamArr() {
  CUDAMALLOCCTRL("&param_arr_", &param_arr_,
                 n_node_ * n_param_ * sizeof(float));
  return 0;
}
...
// to value val
int BaseNeuron::SetScalParam(int i_neuron, int n_neuron, std::string param_name,
                             float val) {
...
// i_neuron, ..., i_neuron + n_neuron -1
// to value val
int BaseNeuron::SetPortParam(int i_neuron, int n_neuron, std::string param_name,
...
// i_neuron, ..., i_neuron + n_neuron -1
// to value val
int BaseNeuron::SetIntVar(int i_neuron, int n_neuron, std::string var_name,
                          int val) {
  ...
  BaseNeuronSetIntArray<<<(n_neuron + 1023) / 1024, 1024>>>(var_pt, n_neuron, 1,
                                                            val);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaDeviceSynchronize());
...
// i_neuron, ..., i_neuron + n_neuron -1
// to value val
int BaseNeuron::SetScalVar(int i_neuron, int n_neuron, std::string var_name,
                           float val) {
  ...
  BaseNeuronSetFloatArray<<<(n_neuron + 1023) / 1024, 1024>>>(var_pt, n_neuron,
                                                              n_var_, val);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaDeviceSynchronize());
...
// i_neuron, ..., i_neuron + n_neuron -1
// to value val
int BaseNeuron::SetPortVar(int i_neuron, int n_neuron, std::string var_name,
...
// to values array[0], ... , array[array_size-1]
// Must be defined in derived classes
int BaseNeuron::SetArrayVar(int i_neuron, int n_neuron, std::string var_name,
...
int BaseNeuron::SetScalVarDistr(int i_neuron, int n_neuron,
  ...
  float *var_pt = GetVarPt(i_neuron, var_name);
  float *d_arr = distribution->getArray(*random_generator_, n_neuron);
  BaseNeuronCopyFloatArray<<<(n_neuron + 1023) / 1024, 1024>>>(var_pt, n_neuron,
                                                               n_var_, d_arr);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaDeviceSynchronize());
  CUDAFREECTRL("d_arr", d_arr);
...
float *BaseNeuron::GetPortParam(int i_neuron, int n_neuron,
  ...
  float *d_param_arr;
  CUDAMALLOCCTRL("&d_param_arr", &d_param_arr,
                 n_neuron * n_port_ * sizeof(float));
  ...
  gpuErrchk(cudaMemcpy(h_param_arr, d_param_arr,
                       n_neuron * n_port_ * sizeof(float),
                       cudaMemcpyDeviceToHost));
  CUDAFREECTRL("d_param_arr", d_param_arr);
...
// i_neuron, ..., i_neuron + n_neuron -1
int *BaseNeuron::GetIntVar(int i_neuron, int n_neuron, std::string var_name) {
  if (!IsIntVar(var_name)) {
    throw ngpu_exception(std::string("Unrecognized integer variable ") +
                         var_name);
  }
  ...
  BaseNeuronGetIntArray<<<(n_neuron + 1023) / 1024, 1024>>>(var_pt, d_var_arr,
                                                            n_neuron, 1, 1);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaDeviceSynchronize());
...
float *BaseNeuron::GetPortVar(int i_neuron, int n_neuron,
  ...
  gpuErrchk(cudaMemcpy(h_var_arr, d_var_arr, n_neuron * n_port_ * sizeof(float),
                       cudaMemcpyDeviceToHost));
  CUDAFREECTRL("d_var_arr", d_var_arr);
...
float *BaseNeuron::GetVarArr() { return var_arr_; }

// return pointer to parameter array
float *BaseNeuron::GetParamArr() { return param_arr_; }
...
float *BaseNeuron::GetVarPt(int i_neuron, std::string var_name,
  ...
  if (IsScalVar(var_name)) {
    ...
    return GetVarArr() + i_neuron * n_var_ + n_scal_var_ + port * n_port_var_ +
           i_vvar;
  } else {
    throw ngpu_exception(std::string("Unrecognized variable ") + var_name);
  }
...
// otherwise return 0
float BaseNeuron::GetSpikeActivity(int i_neuron, long long time_idx) {
  CheckNeuronIdx(i_neuron);
  int i_spike_buffer = i_neuron + i_node_0_;
  long long last_spike_time_idx;

  gpuErrchk(cudaMemcpy(&last_spike_time_idx,
                       &d_LastSpikeTimeIdx[i_spike_buffer], sizeof(long long),
                       cudaMemcpyDeviceToHost));
  if (last_spike_time_idx != time_idx) {
    return 0.0;
  }
  float spike_mul;
  gpuErrchk(cudaMemcpy(&spike_mul, &d_LastSpikeMul[i_spike_buffer],
...
float BaseNeuron::GetSpikeActivity(int i_neuron) {
  ...
  int i_spike_buffer = i_neuron + i_node_0_;
  int Ns;
  gpuErrchk(cudaMemcpy(&Ns, d_SpikeBufferSize + i_spike_buffer, sizeof(int),
                       cudaMemcpyDeviceToHost));
  if (Ns == 0) {
    return 0.0;
  }
  ...
  int is0;
  gpuErrchk(cudaMemcpy(&is0, d_SpikeBufferIdx0 + i_spike_buffer, sizeof(int),
                       cudaMemcpyDeviceToHost));
  int i_arr = is0 * h_NSpikeBuffer + i_spike_buffer; // spike index in array

  int time_idx;
  // get first (most recent) spike from buffer
  gpuErrchk(cudaMemcpy(&time_idx, d_SpikeBufferTimeIdx + i_arr, sizeof(int),
                       cudaMemcpyDeviceToHost));
  if (time_idx != 0) { // neuron is not spiking now
    return 0.0;
  }
  float spike_mul;
  gpuErrchk(cudaMemcpy(&spike_mul, d_SpikeBufferMul + i_arr, sizeof(float),
                       cudaMemcpyDeviceToHost));
...
std::vector<std::string> BaseNeuron::GetIntVarNames() { return int_var_name_; }
...
int BaseNeuron::GetNScalVar() { return n_scal_var_; }

// get number of integer variables
int BaseNeuron::GetNIntVar() { return (int)int_var_name_.size(); }
...
int BaseNeuron::GetNPortVar() { return n_port_var_; }
...
int BaseNeuron::GetNScalParam() { return n_scal_param_; }
...
int BaseNeuron::GetNPortParam() { return n_port_param_; }
...
int BaseNeuron::GetNGroupParam() { return n_group_param_; }
...
int BaseNeuron::GetNArrayVar() { return (int)array_var_name_.size(); }
...
int BaseNeuron::GetNArrayParam() { return (int)array_param_name_.size(); }
...
int BaseNeuron::ActivateRecSpikeTimes(int max_n_rec_spike_times) {
  ...
  if (std::find(int_var_name_.begin(), int_var_name_.end(), s) ==
    ...
    CUDAMALLOCCTRL("&n_rec_spike_times_", &n_rec_spike_times_,
                   n_node_ * sizeof(int));
    ...
    CUDAMALLOCCTRL("&rec_spike_times_", &rec_spike_times_,
                   n_node_ * max_n_rec_spike_times * sizeof(int));
    CUDAMALLOCCTRL("&rec_spike_times_pack_", &rec_spike_times_pack_,
                   n_node_ * max_n_rec_spike_times * sizeof(int));
...
int BaseNeuron::BufferRecSpikeTimes() {
  ...
  gpuErrchk(cudaMemcpy(h_n_rec_spike_times_cumul, n_rec_spike_times_cumul_,
  ...
  if (n_spike_tot > 0) {
    ...
    PackSpikeTimesKernel<<<(n_spike_tot + 1023) / 1024, 1024>>>(
        n_node_, n_rec_spike_times_cumul_, rec_spike_times_,
        rec_spike_times_pack_, n_spike_tot, max_n_rec_spike_times_);

    float *h_rec_spike_times_pack = new float[n_spike_tot];
    gpuErrchk(cudaMemcpy(h_rec_spike_times_pack, rec_spike_times_pack_,
...
int BaseNeuron::GetRecSpikeTimes(int **n_spike_times_pt,
  ...
  for (int i_node = 0; i_node < n_node_; i_node++) {
    ...
    for (uint i_buf = 0; i_buf < spike_times_buffer_.size(); i_buf++) {
      ...
      int n_spike =
          n_spike_times_cumul[i_node + 1] - n_spike_times_cumul[i_node];
