...
// two-dimensional array of maximum delay among the incoming connections of each
// input port of each (local) target node
extern __device__ int *
    *max_input_delay_; // [n_local_nodes][n_input_ports[i_node]]
...
__global__ void initInputSpikeBufferPointersKernel(
    int *n_input_ports, int **max_input_delay, double ***input_spike_buffer,
    int64_t *first_out_connection, int *n_out_connections,
    int64_t *spike_first_connection, int *spike_n_connections,
    float *spike_mul_, int *n_spikes);

// Evaluates the number of input ports of each (local) target node
template <class ConnKeyT, class ConnStructT>
__global__ void getNInputPortsKernel(int64_t n_conn, int *n_input_ports) {
  ...
  // if i_port + 1 is larger than current number of ports evaluated for target
  // node, update it
  atomicMax(&n_input_ports[i_target], i_port + 1);
  // printf("ic1: %ld\tit: %d\tip: %d\n", i_conn, i_target,
  // n_input_ports[i_target]);
}
...
// Evaluates the maximum delay among the incoming connections of each (local)
// target node and receptor port
template <class ConnKeyT, class ConnStructT>
__global__ void getMaxInputDelayKernel(int64_t n_conn, int **max_input_delay) {
  int64_t i_conn = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
  if (i_conn >= n_conn) {
    return;
  }
  ...
  // atomic operation to avoid conflicts in memory access
  // if delay is larger than current maximum evaluated for target node, update
  // the maximum
  atomicMax(&max_input_delay[i_target][i_port], i_delay + 1);
}

template <class ConnKeyT, class ConnStructT>
__global__ void testMaxInputDelayKernel(int64_t n_conn, int **max_input_delay,
                                        int n_local_nodes, int max_n_ports) {
  int64_t i_conn = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
  if (i_conn >= n_conn) {
    return;
  }
...
__global__ void getFirstOutConnectionKernel(int64_t n_conn,
  ...
  if (i_source < 0 || i_source >= n_nodes) {
    printf("Error in getFirstOutConnectionKernel, host %d, i_source %d, "
           "n_nodes %d\n",
  ...
  if (i_conn >= 1) {
    ...
    // get references to key-structure pair representing the connection
    ConnKeyT &conn_key_prev =
        ((ConnKeyT **)ConnKeyArray)[i_block_prev][i_block_conn_prev];

    int i_source_prev = getConnSource<ConnKeyT>(conn_key_prev);
    if (i_source_prev < 0 || i_source_prev >= n_nodes) {
      printf("Error in getFirstOutConnectionKernel, host %d, i_source_prev %d, "
             "n_nodes %d\n",
...
__global__ void getFirstOutConnectionKernel(inode_t i_node_0, int64_t i_conn_0,
  ...
  if (i_source < i_node_0 || i_source >= i_node_0 + n_nodes) {
    printf("Error in getFirstOutConnectionKernel, host %d, i_node_0 %d, "
           "i_source %d, n_nodes %d\n",
...
template <class ConnKeyT>
__global__ void getNOutConnectionsKernel(inode_t n_node, int64_t n_conn,
                                         int64_t *first_out_connection,
...
template <class ConnKeyT, class ConnStructT>
__global__ void deliverSpikesKernel(int64_t n_conn) {
  ...
  int64_t i_conn1 =
      i_conn0 + input_spike_buffer_ns::spike_n_connections_[i_spike];

  float mul = input_spike_buffer_ns::spike_mul_[i_spike];
  for (int64_t i_conn = i_conn0 + threadIdx.x; i_conn < i_conn1;
       i_conn += blockDim.x) {
    ...
    // printf("i_spike: %d\ti_conn0: %lld\ti_target: %d\ti_port: %d\ti_slot:
    // %d\tweight: %f\tmul: %f\n", i_spike, i_conn0, i_target, i_port, i_slot,
    // weight, mul );
  }
}

template <class ConnKeyT, class ConnStructT>
__global__ void deliverSpikesNoNOutConnKernel(int64_t n_conn) {
  ...
  for (int64_t i_conn = i_conn1; i_conn < n_conn; i_conn += blockDim.x) {
    ...
    int n_slots = max_input_delay_[i_target][i_port];
    int i_slot = (NESTGPUTimeIdx + i_delay) % n_slots;
    // printf("i_conn: %ld\ti_source: %d\ti_target: %d\ti_port: %d\ti_delay:
    // %d\tw: %f\tn_slots: %d\n", i_conn, i_source, i_target, i_port, i_delay,
    // weight, n_slots); printf("n_slots: %d\ti_delay: %d\ttime_idx:
...
__global__ void sendDirectSpikeKernel(curandState *curand_state,
                                      long long time_idx, float *mu_arr,
                                      ConnKeyT *poiss_key_array, int64_t n_conn,
                                      int64_t i_conn_0, int64_t block_size,
...
__global__ void GetInputSpikes(inode_t i_node0, inode_t n_nodes, int n_port,
                               int n_var, float *port_weight_arr,
                               int port_weight_arr_step,
                               int port_weight_port_step, float *port_input_arr,
...
template <class ConnKeyT, class ConnStructT>
int ConnectionTemplate<ConnKeyT, ConnStructT>::initInputSpikeBuffer(
    inode_t n_local_nodes, inode_t n_nodes, int max_remote_spike_num) {
  // allocate array of number of input ports of each (local) target node
  CUDAMALLOCCTRL("&d_n_input_ports_", &d_n_input_ports_,
                 n_local_nodes * sizeof(int));
  gpuErrchk(cudaMemset(d_n_input_ports_, 0, n_local_nodes * sizeof(int)));

  if (n_conn_ > 0) {
    getNInputPortsKernel<ConnKeyT, ConnStructT>
        <<<(n_conn_ + 1023) / 1024, 1024>>>(n_conn_, d_n_input_ports_);
    DBGCUDASYNC;
  }

  CUDAMALLOCCTRL("&d_n_input_ports_cumul_", &d_n_input_ports_cumul_,
                 (n_local_nodes + 1) * sizeof(int64_t));
  // Determine temporary device storage requirements
  void *d_temp_storage = NULL;
  size_t temp_storage_bytes = 0;
  //<BEGIN-CLANG-TIDY-SKIP>//
  cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes,
                                d_n_input_ports_, d_n_input_ports_cumul_,
                                n_local_nodes + 1);
  //<END-CLANG-TIDY-SKIP>//
  ...
  gpuErrchk(cudaMemcpy(&n_input_ports_tot_,
                       &d_n_input_ports_cumul_[n_local_nodes], sizeof(int64_t),
                       cudaMemcpyDeviceToHost));

  // allocate (two-dimensional) array of maximum delay among the incoming
  // connections of each input port of each (local) target node
  CUDAMALLOCCTRL("&d_max_input_delay_", &d_max_input_delay_,
                 n_local_nodes * sizeof(int *));

  if (n_input_ports_tot_ > 0) {
    // allocate one-dimensional array of maximum delay among the incoming
    // connections of each input port of each (local) target node
    CUDAMALLOCCTRL("&d_max_input_delay_1d_", &d_max_input_delay_1d_,
                   n_input_ports_tot_ * sizeof(int));
    gpuErrchk(
        cudaMemset(d_max_input_delay_1d_, 0, n_input_ports_tot_ * sizeof(int)));

    initMaxInputDelayArrayKernel<<<(n_local_nodes + 1023) / 1024, 1024>>>(
        n_local_nodes, d_max_input_delay_, d_max_input_delay_1d_,
        d_n_input_ports_cumul_);
    DBGCUDASYNC;
  }

  if (n_conn_ > 0) {
    // testMaxInputDelayKernel< ConnKeyT, ConnStructT > <<< ( n_conn_ + 1023 ) /
    // 1024, 1024 >>>(n_conn_, d_max_input_delay_, n_local_nodes, 2 ); CUDASYNC;
    getMaxInputDelayKernel<ConnKeyT, ConnStructT>
        <<<(n_conn_ + 1023) / 1024, 1024>>>(n_conn_, d_max_input_delay_);
    DBGCUDASYNC;
  }
  n_input_spike_buffer_tot_ = 0;
  if (n_input_ports_tot_ > 0) {
    // Evaluates the cumulative sum of of maximum delay among the incoming
    // connections of each input port of each (local) target node
    CUDAMALLOCCTRL("&d_max_input_delay_cumul_", &d_max_input_delay_cumul_,
                   (n_input_ports_tot_ + 1) * sizeof(int64_t));
    // Determine temporary device storage requirements
    d_temp_storage = NULL;
    temp_storage_bytes = 0;
    //<BEGIN-CLANG-TIDY-SKIP>//
    cub::DeviceScan::ExclusiveSum(
        d_temp_storage, temp_storage_bytes, d_max_input_delay_1d_,
        d_max_input_delay_cumul_, n_input_ports_tot_ + 1);
    //<END-CLANG-TIDY-SKIP>//
  ...
  if (n_input_spike_buffer_tot_ > 0) {
    ...
    CUDAMALLOCCTRL("&d_input_spike_buffer_1d_", &d_input_spike_buffer_1d_,
                   n_input_spike_buffer_tot_ * sizeof(double));
    gpuErrchk(cudaMemset(d_input_spike_buffer_1d_, 0,
                         n_input_spike_buffer_tot_ * sizeof(double)));

    CUDAMALLOCCTRL("&d_input_spike_buffer_2d_", &d_input_spike_buffer_2d_,
                   n_input_ports_tot_ * sizeof(double *));
    initInputSpikeBuffer2DKernel<<<(n_input_ports_tot_ + 1023) / 1024, 1024>>>(
        n_input_ports_tot_, d_input_spike_buffer_1d_, d_input_spike_buffer_2d_,
        d_max_input_delay_cumul_);
    DBGCUDASYNC;

    CUDAMALLOCCTRL("&d_input_spike_buffer_", &d_input_spike_buffer_,
                   n_local_nodes * sizeof(double **));

    initInputSpikeBufferKernel<<<(n_local_nodes + 1023) / 1024, 1024>>>(
        n_local_nodes, d_input_spike_buffer_, d_input_spike_buffer_2d_,
        d_n_input_ports_cumul_);
    DBGCUDASYNC;
  }
  ...
  if (!first_out_conn_in_device_ || check_first_out_connection_) {
    ...
    if (!check_first_out_connection_) {
      // write n_local_nodes in the ConnKeyT representation with minimum delay
      // -> conn_key
      ConnKeyT conn_key = 0;
      setConnSource(conn_key, n_local_nodes);
      CUDAMALLOCCTRL("&d_first_out_connection_", &d_first_out_connection_,
                     sizeof(int64_t) * n_local_nodes);
      initFirstOutConnectionKernel<<<(n_local_nodes + 1023) / 1024, 1024>>>(
          n_local_nodes, d_first_out_connection_);
      DBGCUDASYNC;
      if (n_conn_ > 0) {
        // Find number of connections with key < conn_key, i.e. source index <
        // n_local_nodes
        n_local_conn = search_block_array_down<ConnKeyT>(
            &conn_key_vect_[0], n_conn_, conn_block_size_, conn_key);
        // allocate array of indexes of first outgoing connection from each
        // source node
        if (n_local_conn > 0) {
          // Evaluates the index of the first outgoing connection of each local
          // source node
          getFirstOutConnectionKernel<ConnKeyT>
              <<<(n_local_conn + 1023) / 1024, 1024>>>(
                  n_local_conn, d_first_out_connection_, n_local_nodes,
                  this_host_);
          DBGCUDASYNC;
        }
      }
  ...
  if (first_out_conn_in_device_ || check_first_out_connection_) {
    // allocate array of indexes of first outgoing connection from each source
    // node
    CUDAMALLOCCTRL("&d_first_out_connection_", &d_first_out_connection_,
                   sizeof(int64_t) * n_nodes);

    initFirstOutConnectionKernel<<<(n_nodes + 1023) / 1024, 1024>>>(
        n_nodes, d_first_out_connection_);
    DBGCUDASYNC;
    if (n_conn_ > 0) {
      // Evaluates the index of the first outgoing connection of each source
      // node
      getFirstOutConnectionKernel<ConnKeyT><<<(n_conn_ + 1023) / 1024, 1024>>>(
          n_conn_, d_first_out_connection_, n_nodes, this_host_);
      DBGCUDASYNC;
    }
  }
  // else {
  //   gpuErrchk( cudaMemset( d_first_out_connection_, 0, sizeof( int64_t ) *
  //   n_nodes ) );
  // }
  //  allocate array of the first connection of each spike emitted at current
  //  time step and array of spike multiplicity temporary size is n_local_nodes,
  //  in the future evaluate [n_all_nodes*time_resolution*avg_max_firing_rate]
  CUDAMALLOCCTRL("&d_spike_first_connection_", &d_spike_first_connection_,
                 max_remote_spike_num * sizeof(int64_t));
  CUDAMALLOCCTRL("&d_spike_mul_", &d_spike_mul_,
                 max_remote_spike_num * sizeof(float));
  // number of spikes emitted at current time step
  ...
  if (have_n_out_conn_) {
    if (first_out_conn_in_device_ || check_first_out_connection_) {
      ...
      CUDAMALLOCCTRL("&d_n_out_connections_", &d_n_out_connections_,
                     sizeof(int) * n_nodes);
      ...
      if (n_conn_ > 0) {
        ...
        getNOutConnectionsKernel<ConnKeyT><<<(n_nodes + 1023) / 1024, 1024>>>(
            n_nodes, n_conn_, d_first_out_connection_, d_n_out_connections_);
        DBGCUDASYNC;
      }
    } else {
      // allocate array of number of outgoing connections from each node
      CUDAMALLOCCTRL("&d_n_out_connections_", &d_n_out_connections_,
                     sizeof(int) * n_local_nodes);
      gpuErrchk(
          cudaMemset(d_n_out_connections_, 0, sizeof(int) * n_local_nodes));
      if (n_local_conn > 0) {
        // Evaluates the number of outgoing connection from each source node
        getNOutConnectionsKernel<ConnKeyT>
            <<<(n_local_nodes + 1023) / 1024, 1024>>>(
                n_local_nodes, n_local_conn, d_first_out_connection_,
                d_n_out_connections_);
        DBGCUDASYNC;
      }
    }
    // allocate array of number of connections through which each spike emitted
    // at current time step should be delivered temporary size is n_local_nodes,
    // in the future evaluate [n_all_nodes*time_resolution*avg_max_firing_rate]
    CUDAMALLOCCTRL("&d_spike_n_connections_", &d_spike_n_connections_,
                   max_remote_spike_num * sizeof(int));
  }
  ...
  if (first_out_conn_in_device_ && check_first_out_connection_) {
    if (n_nodes > n_local_nodes) {
      std::cout << "Checking first_out_connection on host " << this_host_
                << std::endl;
      ...
      gpuErrchk(cudaMemcpy(&check_first_out_connection[0],
                           d_first_out_connection_ + n_local_nodes,
                           n_image_nodes * sizeof(int64_t),
                           cudaMemcpyDeviceToHost));
      for (uint i = 0; i < n_image_nodes; i++) {
        if (check_first_out_connection[i] != h_first_out_connection_[i]) {
          throw ngpu_exception(
              std::string("Error checking first_out_connection on host ") +
              std::to_string(this_host_));
        }
      }
      std::cout << "Finished checking first_out_connection on host "
                << this_host_ << " OK" << std::endl;

      if (have_n_out_conn_) {
        std::cout << "Checking n_out_connections on host " << this_host_
                  << std::endl;
        std::vector<int> check_n_out_connections(n_image_nodes);
        // copy d_n_out_connections_ to CPU memory
        gpuErrchk(cudaMemcpy(
            &check_n_out_connections[0], d_n_out_connections_ + n_local_nodes,
            n_image_nodes * sizeof(int), cudaMemcpyDeviceToHost));
        for (uint i = 0; i < n_image_nodes; i++) {
          if (check_n_out_connections[i] != h_n_out_connections_[i]) {
            throw ngpu_exception(
                std::string("Error checking n_out_connections on host ") +
                std::to_string(this_host_));
          }
        }
        std::cout << "Finished checking n_out_connections on host "
                  << this_host_ << " OK" << std::endl;
      }
    }
  }
  ...
  initInputSpikeBufferPointersKernel<<<1, 1>>>(
      d_n_input_ports_, d_max_input_delay_, d_input_spike_buffer_,
      d_first_out_connection_, d_n_out_connections_, d_spike_first_connection_,
      d_spike_n_connections_, d_spike_mul_, d_n_spikes_);
  DBGCUDASYNC;
  ...
  gpuErrchk( cudaMemcpy(h_input_spike_buffer, d_input_spike_buffer_,
  n_local_nodes*sizeof( double** ), cudaMemcpyDeviceToHost ) );

  int **h_max_input_delay = new int*[n_local_nodes];
  gpuErrchk( cudaMemcpy(h_max_input_delay, d_max_input_delay_,
  n_local_nodes*sizeof( int* ), cudaMemcpyDeviceToHost )
  );

  int *h_n_input_ports = new int[n_local_nodes];
  gpuErrchk( cudaMemcpy(h_n_input_ports, d_n_input_ports_, n_local_nodes*sizeof(
  ...
  for (uint i_target=0; i_target<n_local_nodes; i_target++) {
    ...
    gpuErrchk( cudaMemcpy(h_input_spike_buffer_1,
  h_input_spike_buffer[i_target], n_ports*sizeof( double* ),
  cudaMemcpyDeviceToHost ) );

    int *h_max_input_delay_1 = new int[n_ports];
    gpuErrchk( cudaMemcpy(h_max_input_delay_1, h_max_input_delay[i_target],
  n_ports*sizeof( int ), cudaMemcpyDeviceToHost ) ); for (int i_port=0;
  i_port<n_ports; i_port++) { int n_delays = h_max_input_delay_1[i_port];
  printf("\t\tport: %d\tn_delays: %d\t&input_spike_buffer[][][]: %lld\n",
  i_port, n_delays, (long long)&h_input_spike_buffer_1[i_port]);
    }
  ...
  gpuErrchk( cudaMemcpy(h_first_out_connection, d_first_out_connection_,
  n_local_nodes*sizeof( int64_t ), cudaMemcpyDeviceToHost ) ); for (uint i=0;
  i<n_local_nodes; i++) { printf("i_node: %d\tfirst conn: %ld\n", i,
...
template <class ConnKeyT, class ConnStructT>
int ConnectionTemplate<ConnKeyT, ConnStructT>::deliverSpikes() {
  int n_spikes;
  gpuErrchk(
      cudaMemcpy(&n_spikes, d_n_spikes_, sizeof(int), cudaMemcpyDeviceToHost));

  if (n_spike_from_host_ > 0) {
    gpuErrchk(cudaMemcpyAsync(
        d_spike_first_connection_ + n_spikes, &h_spike_first_connection_[0],
        n_spike_from_host_ * sizeof(int64_t), cudaMemcpyHostToDevice));
    DBGCUDASYNC;
    gpuErrchk(cudaMemcpyAsync(
        d_spike_n_connections_ + n_spikes, &h_spike_n_connections_[0],
        n_spike_from_host_ * sizeof(int), cudaMemcpyHostToDevice));
    DBGCUDASYNC;
    // for now spike multiplicity from remote nodes is assumed to be one
    initSpikeMulKernel<<<(n_spike_from_host_ + 1023) / 1024, 1024>>>(
        n_spike_from_host_, d_spike_mul_ + n_spikes);
    DBGCUDASYNC;

    n_spikes += n_spike_from_host_;
    gpuErrchk(cudaMemcpyAsync(d_n_spikes_, &n_spikes, sizeof(int),
  ...
  if (n_spikes > 0) {
    if (have_n_out_conn_) {
      ...
      deliverSpikesNoNOutConnKernel<ConnKeyT, ConnStructT>
          <<<n_spikes, 1024>>>(n_conn_);
      DBGCUDASYNC;
    }
  }
...
int ConnectionTemplate<ConnKeyT, ConnStructT>::sendDirectSpikes(
    long long time_idx, int64_t i_conn0, int64_t n_dir_conn, inode_t n_node,
  ...
  sendDirectSpikeKernel<ConnKeyT, ConnStructT><<<numBlocks, 1024>>>(
      d_curand_state, time_idx, d_mu_arr, (ConnKeyT *)d_poiss_key_array,
      n_dir_conn, i_conn0, conn_block_size_, n_node, max_delay);
...
template <class ConnKeyT, class ConnStructT>
int ConnectionTemplate<ConnKeyT, ConnStructT>::_setSpikeBufferAlgo(
    int spike_buffer_algo) {
...
template <class ConnKeyT, class ConnStructT>
int ConnectionTemplate<ConnKeyT, ConnStructT>::getSpikeBufferAlgo() {
