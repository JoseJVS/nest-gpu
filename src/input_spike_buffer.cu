...
__global__ void initInputSpikeBufferPointersKernel(
    ...
    int64_t *spike_first_connection, int *spike_n_connections, float *spike_mul,
    int *n_spikes) {
...
__global__ void initInputSpikeBuffer2DKernel(int64_t n_input_ports_tot,
  ...
  input_spike_buffer_2d[i_port_abs] =
      input_spike_buffer_1d + max_input_delay_cumul[i_port_abs];
}
...
__global__ void initInputSpikeBufferKernel(inode_t n_local_nodes,
  ...
  input_spike_buffer[i_target] =
      input_spike_buffer_2d + n_input_ports_cumul[i_target];
}
...
__global__ void initFirstOutConnectionKernel(inode_t n_nodes,
...
__global__ void GetInputSpikes(inode_t i_node0, inode_t n_nodes, int n_port,
  ...
  if (i_target_rel < n_nodes && i_port < n_port) {
    ...
    if (i_port >= n_input_ports_[i_target]) { // number of ports actually
                                              // reached by connections
      return;
    }
    ...
    int port_input =
        i_target_rel * port_input_arr_step + port_input_port_step * i_port;
    int port_weight =
        i_target_rel * port_weight_arr_step + port_weight_port_step * i_port;
    double d_val = (double)port_input_arr[port_input] +
                   spike_input * port_weight_arr[port_weight];
    // if (i_target==37 && d_val > 700) {
    //   printf("port_input_arr: %lf\tspike_input: %lf\tport_weight_arr:
    //   %f\td_val: %lf\n",
    //      port_input_arr[ port_input ], spike_input, port_weight_arr[
    //      port_weight ], d_val);
