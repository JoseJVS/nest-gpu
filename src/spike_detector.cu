...
enum { i_spike_detector_hold_spike_mul = 0, N_SPIKE_DETECTOR_SCAL_PARAM };
...
const std::string spike_detector_scal_var_name[N_SPIKE_DETECTOR_SCAL_VAR] = {
    "input_spike_mul", "spike_mul"};

__global__ void spike_detector_UpdateKernel(int i_node_0, int n_node,
                                            float *var_arr, float *param_arr,
                                            int n_var, int n_param) {
  int irel_node = threadIdx.x + blockIdx.x * blockDim.x;
  if (irel_node < n_node) {
    float *input_spike_mul_pt =
        var_arr + irel_node * n_var + i_spike_detector_input_spike_mul;
    float *spike_mul_pt =
        var_arr + irel_node * n_var + i_spike_detector_spike_mul;
    float *hold_spike_mul_pt =
        param_arr + irel_node * n_param + i_spike_detector_hold_spike_mul;
...
int spike_detector::Init(int i_node_0, int n_node, int /*n_port*/,
  ...
  gpuErrchk(cudaMemcpy(port_weight_arr_, &input_weight, sizeof(float),
