...
template <class T>
void cudaReusableAlloc(void *d_storage, int64_t &st_bytes, T **variable_pt,
                       const int64_t &num_elems, const size_t &elem_size) {
...
template <class KeyT, class ValueT> struct key_value {
  KeyT key;
  ValueT value;
};

template <class KeyT, class ValueT> struct contiguous_key_value {
  KeyT *key_pt;
  ValueT *value_pt;
  position_t offset;
  position_t size;
...
template <class KeyT, class ValueT> struct regular_block_key_value {
  KeyT **h_key_pt;
  ValueT **h_value_pt;
  KeyT **key_pt;
  ValueT **value_pt;
...
void array_GPUtoCPUCopyContent(contiguous_key_value<KeyT, ValueT> &target_arr,
  ...
  gpuErrchk(cudaMemcpy(target_arr.key_pt + target_arr.offset,
                       ...
                       source_arr.size * sizeof(KeyT), cudaMemcpyDeviceToHost));
...
void array_CPUtoGPUCopyContent(contiguous_key_value<KeyT, ValueT> &target_arr,
  ...
  gpuErrchk(cudaMemcpy(target_arr.key_pt + target_arr.offset,
                       source_arr.key_pt + source_arr.offset,
                       source_arr.size * sizeof(KeyT), cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpy(target_arr.value_pt + target_arr.offset,
                       source_arr.value_pt + source_arr.offset,
                       source_arr.size * sizeof(ValueT),
...
template <class KeyT, class ValueT>
void array_GPUSort(contiguous_key_value<KeyT, ValueT> &arr_in, void *d_storage,
                   int64_t &ext_st_bytes) {
  ext_st_bytes = 0;
  ...
  cub::DeviceRadixSort::SortPairs(
      NULL, sort_storage_bytes, arr_in.key_pt + arr_in.offset, arr_out.key_pt,
      arr_in.value_pt + arr_in.offset, arr_out.value_pt, num_elems);
  //<END-CLANG-TIDY-SKIP>//
  ...
  if (d_storage != NULL) {
    ...
    //<BEGIN-CLANG-TIDY-SKIP>//
    cub::DeviceRadixSort::SortPairs(
        d_sort_storage, sort_storage_bytes, arr_in.key_pt + arr_in.offset,
        arr_out.key_pt, arr_in.value_pt + arr_in.offset, arr_out.value_pt,
        num_elems);
    //<END-CLANG-TIDY-SKIP>//
    ...
    gpuErrchk(cudaMemcpy(arr_in.value_pt + arr_in.offset, arr_out.value_pt,
                         num_elems * sizeof(ValueT), cudaMemcpyDeviceToDevice));
  }
...
template <class KeyT, class ValueT>
__device__ KeyT getKey(regular_block_key_value<KeyT, ValueT> &arr,
                       position_t i) {
  position_t position = arr.offset + i;
...
__device__ void setElem(regular_block_key_value<KeyT, ValueT> &arr,
                        position_t i, const key_value<KeyT, ValueT> &kv) {
  position_t position = arr.offset + i;
...
template <class ElementT> struct contiguous_array {
  ElementT *data_pt;
  position_t offset;
  position_t size;
};

template <class ElementT> struct regular_block_array {
  ElementT **h_data_pt;
  ElementT **data_pt;
  position_t block_size;
  position_t offset;
...
template <class ElementT> ElementT *getKeyPt(contiguous_array<ElementT> &arr) {
...
template <class ElementT> void array_Sort(contiguous_array<ElementT> &arr) {
  // sort array
...
template <class ElementT>
void array_GPUSort(contiguous_array<ElementT> &arr_in, void *d_storage,
                   int64_t &ext_st_bytes) {
  ext_st_bytes = 0;
  int num_elems = arr_in.size;
...
__device__ void search_block_up(ArrayT array, position_t size, KeyT val,
  ...
  if (tid == 0) {
    ...
    // (int)size); printf("search_block_up arr[0]: %d arr[n-1] %d val %d\n",
    // (int)getKey(array, 0), 	   (int)getKey(array, size-1), (int)val);
  }
  __syncthreads();
  while ((right - left) > blockDim.x) {
    ...
    if (val0 <= val && val1 > val) {
      left = pos0;
      right = pos1;
      // printf("search_block_up narrowing bid:%d tid:%d sa0:%d sa1:%d l:%d
      // r:%d\n", (int)blockIdx.x,
  ...
  if ((right - left) > 1 && tid < right - left) {
    ...
    position_t pos1 =
        left + tid + 1; // for tid = right - left - 1 : pos1 = right
    KeyT val0 = getKey(array, pos0);
    KeyT val1 = getKey(array, pos1);
    if (val0 <= val && val1 > val) {
      left = pos0;
...
template <class KeyT, class ArrayT, uint bsize>
__global__ void search_multi_up_kernel(ArrayT *subarray, KeyT *val_pt,
                                       position_t *num_up,
                                       position_t *sum_num_up) {
  int bid = blockIdx.x;
  KeyT val = *val_pt;
  search_block_up<KeyT, ArrayT, bsize>(subarray[bid], subarray[bid].size, val,
                                       &num_up[bid]);
  if (threadIdx.x == 0) {
...
__device__ void search_block_down(ArrayT array, position_t size, KeyT val,
  ...
  if (tid == 0) {
    ...
    // (int)size); printf("search_block_down arr[0]: %d arr[n-1] %d val %d\n",
    // (int)getKey(array, 0), 	   (int)getKey(array, size-1), (int)val);
  }
  __syncthreads();
  while ((right - left) > blockDim.x) {
    ...
    if (val0 < val && val1 >= val) {
      left = pos0;
      right = pos1;
      // printf("search_block_down narrowing bid:%d tid:%d sa0:%d sa1:%d l:%d
      // r:%d\n", (int)blockIdx.x,
      //     (int)tid, (int)val0, (int)val1, (int)left, (int)right);
    }
    __syncthreads();
  ...
  if ((right - left) > 1 && tid < right - left) {
    ...
    if (val0 < val && val1 >= val) {
      left = pos0;
      right = pos1;
      // printf("search_block_down last bid:%d tid:%d sa0:%d sa1:%d l:%d
      // r:%d\n", (int)blockIdx.x,
      //     (int)tid, (int)val0, (int)val1, (int)left, (int)right);
    }
  }
...
template <class KeyT, class ArrayT, uint bsize>
__global__ void search_multi_down_kernel(ArrayT *subarray, KeyT *val_pt,
                                         position_t *num_down,
                                         position_t *sum_num_down) {
  int bid = blockIdx.x;
  KeyT val = *val_pt;
  search_block_down<KeyT, ArrayT, bsize>(subarray[bid], subarray[bid].size, val,
                                         &num_down[bid]);
  if (threadIdx.x == 0) {
...
__global__ void max_diff_kernel(position_t *m_u, position_t *m_d,
                                ...
                                position_t *max_diff, int *arg_max) {
...
__global__ void prefix_scan(ElementT *array_in, ElementT *array_out, uint k,
                            uint n) {
...
__global__ void search_down(ElementT *array, position_t size, ElementT val,
                            position_t *num_down) {
  contiguous_array<ElementT> arr;
  arr.data_pt = array;
  arr.offset = 0;
  search_block_down<ElementT, contiguous_array<ElementT>, bsize>(arr, size, val,
                                                                 num_down);
}
...
// in a sorted array array[i+1]>=array[i]
template <class ElementT, uint bsize>
__global__ void search_up(ElementT *array, position_t size, ElementT val,
                          position_t *num_up) {
  contiguous_array<ElementT> arr;
  arr.data_pt = array;
  arr.offset = 0;
  search_block_up<ElementT, contiguous_array<ElementT>, bsize>(arr, size, val,
                                                               num_up);
}

template <class KeyT, class ArrayT, uint bsize>
int search_multi_up(ArrayT *d_subarray, uint k, KeyT *d_val_pt,
                    position_t *d_num_up, position_t *d_sum_num_up) {
  gpuErrchk(cudaMemsetAsync(d_sum_num_up, 0, sizeof(position_t)));
  search_multi_up_kernel<KeyT, ArrayT, bsize>
      <<<k, bsize>>>(d_subarray, d_val_pt, d_num_up, d_sum_num_up);
  DBGCUDASYNC
...
template <class KeyT, class ArrayT, uint bsize>
int search_multi_down(ArrayT *d_subarray, uint k, KeyT *d_val_pt,
                      position_t *d_num_down, position_t *d_sum_num_down) {
  gpuErrchk(cudaMemsetAsync(d_sum_num_down, 0, sizeof(position_t)));
  search_multi_down_kernel<KeyT, ArrayT, bsize>
      <<<k, bsize>>>(d_subarray, d_val_pt, d_num_down, d_sum_num_down);
...
__global__ void threshold_range_kernel(ArrayT *subarray,
                                       position_t tot_part_size, uint k,
                                       KeyT *t_u, KeyT *t_d) {
...
__global__ void eval_t_tilde_kernel(ArrayT *subarray, position_t *m_u,
                                    position_t *m_d, int *arg_max,
                                    KeyT *t_tilde) {
  int i = *arg_max;
...
template <class KeyT, class ArrayT>
__global__ void case2_extra_elems_kernel(ArrayT *subarray, uint k,
                                         position_t *m_d, position_t *m_u,
                                         KeyT *extra_elem, int *extra_elem_idx,
                                         int *n_extra_elems) {
  int i = threadIdx.x;
  if (i == 0) {
    *n_extra_elems = 0;
...
__global__ void
extract_partitions_kernel(ArrayT *subarray, uint k, position_t *part_size,
                          position_t *part_size_cumul, AuxArrayT aux_array) {
...
template <class KeyT>
void contiguousTranslate(contiguous_array<KeyT> &arr, position_t transl,
                         char *d_buffer, position_t buffer_size) {
  ...
  if (transl >= elem_num) {
    gpuErrchk(cudaMemcpyAsync(&arr.data_pt[t_pos0], &arr.data_pt[s_pos0],
                              elem_num * sizeof(KeyT),
                              cudaMemcpyDeviceToDevice));
  } else {
    GPUMemCpyBuffered((char *)&arr.data_pt[t_pos0],
                      (char *)&arr.data_pt[s_pos0], elem_num * sizeof(KeyT),
...
void contiguousTranslate(contiguous_key_value<KeyT, ValueT> &arr,
  ...
  if (transl >= elem_num) {
    gpuErrchk(cudaMemcpyAsync(&arr.key_pt[t_pos0], &arr.key_pt[s_pos0],
                              elem_num * sizeof(KeyT),
                              cudaMemcpyDeviceToDevice));
    gpuErrchk(cudaMemcpyAsync(&arr.value_pt[t_pos0], &arr.value_pt[s_pos0],
                              elem_num * sizeof(ValueT),
                              cudaMemcpyDeviceToDevice));
  } else {
    GPUMemCpyBuffered((char *)&arr.key_pt[t_pos0], (char *)&arr.key_pt[s_pos0],
                      elem_num * sizeof(KeyT), d_buffer, buffer_size);
    GPUMemCpyBuffered((char *)&arr.value_pt[t_pos0],
                      (char *)&arr.value_pt[s_pos0], elem_num * sizeof(ValueT),
                      d_buffer, buffer_size);
  }
  arr.offset += transl;
}
...
template <class KeyT, class ValueT>
void CopyRegion(regular_block_key_value<KeyT, ValueT> &arr, int t_ib,
                position_t t_j0, int s_ib, position_t s_j0, position_t elem_num,
                char *d_buffer, position_t buffer_size) {

  position_t transl = t_j0 - s_j0;
  if (t_ib != s_ib || transl >= elem_num) {
    gpuErrchk(
        cudaMemcpyAsync(&arr.h_key_pt[t_ib][t_j0], &arr.h_key_pt[s_ib][s_j0],
                        elem_num * sizeof(KeyT), cudaMemcpyDeviceToDevice));
    gpuErrchk(cudaMemcpyAsync(
        &arr.h_value_pt[t_ib][t_j0], &arr.h_value_pt[s_ib][s_j0],
        elem_num * sizeof(ValueT), cudaMemcpyDeviceToDevice));
  } else {
...
template <class KeyT>
void CopyRegion(regular_block_array<KeyT> &arr, int t_ib, position_t t_j0,
                int s_ib, position_t s_j0, position_t elem_num, char *d_buffer,
                position_t buffer_size) {

  position_t transl = t_j0 - s_j0;
  if (t_ib != s_ib || transl >= elem_num) {
    gpuErrchk(
        cudaMemcpyAsync(&arr.h_data_pt[t_ib][t_j0], &arr.h_data_pt[s_ib][s_j0],
                        elem_num * sizeof(KeyT), cudaMemcpyDeviceToDevice));
  } else {
...
void regularBlockTranslate(ArrayT &arr, position_t transl, char *d_buffer,
                           position_t buffer_size) {
  position_t elem_num = arr.size;
...
template <class KeyT>
void Translate(contiguous_array<KeyT> &arr, position_t transl, char *d_buffer,
               position_t buffer_size) {
  contiguousTranslate(arr, transl, d_buffer, buffer_size);
}

template <class KeyT, class ValueT>
void Translate(contiguous_key_value<KeyT, ValueT> &arr, position_t transl,
               char *d_buffer, position_t buffer_size) {
  contiguousTranslate(arr, transl, d_buffer, buffer_size);
}

template <class KeyT>
void Translate(regular_block_array<KeyT> &arr, position_t transl,
               char *d_buffer, position_t buffer_size) {
  regularBlockTranslate(arr, transl, d_buffer, buffer_size);
}

template <class KeyT, class ValueT>
void Translate(regular_block_key_value<KeyT, ValueT> &arr, position_t transl,
               char *d_buffer, position_t buffer_size) {
  regularBlockTranslate(arr, transl, d_buffer, buffer_size);
}

template <class ArrayT>
void repack(ArrayT *h_subarray, uint k, position_t *part_size, char *d_buffer,
            position_t buffer_size) {
...
template <class T>
int64_t search_block_array_down(T **array, int64_t array_size,
                                int64_t block_size, T val, int64_t pos0 = 0) {
  int ib0 = (int)(pos0 / block_size);
  int ib1 = (int)((array_size - 1) / block_size);

  int64_t n_down = array_size; // this is the result if the loop on the blocks
                               // reaches the end without break
  ...
  for (int ib = ib0; ib <= ib1; ib++) {
    ...
    gpuErrchk(cudaMemcpy(&first_val_in_block, &array[ib][i0], sizeof(T),
                         cudaMemcpyDeviceToHost));
    ...
    int64_t i1 =
        (ib == ib1) ? ((array_size - 1) % block_size) : (block_size - 1);
    gpuErrchk(cudaMemcpy(&last_val_in_block, &array[ib][i1], sizeof(T),
                         cudaMemcpyDeviceToHost));
    if (last_val_in_block == val) {
      ...
      search_down<T, 1024>
          <<<1, 1024>>>(array[ib] + i0, i1 - i0 + 1, val, d_position);
      DBGCUDASYNC;
      // Copy position from GPU to CPU memory
      int64_t h_position;
      gpuErrchk(cudaMemcpy(&h_position, d_position, sizeof(int64_t),
                           cudaMemcpyDeviceToHost));
      n_down = ib * block_size + i0 + h_position;
      // check if found
      if (n_down < pos0 || n_down > array_size) {
        printf(
            "ib: %d\tblock_size: %ld\ti0: %ld\th_position: %ld\tn_down: %ld\n",
            ib, block_size, i0, h_position, n_down);
        throw ngpu_exception(
            "Inconsistent position in search_block_array_down");
      }
      break;
    }
  }
