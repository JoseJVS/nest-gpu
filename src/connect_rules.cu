...
int ConnSpec::Init(int rule, int degree /*=0*/) {
  ...
  if ((rule == ALL_TO_ALL || rule == ONE_TO_ONE) && (degree != 0)) {
    throw ngpu_exception(std::string("Connection rule ") +
                         conn_rule_name[rule] + " does not have a degree");
  }
  rule_ = rule;
...
ConnSpec::ConnSpec(int rule, int degree /*=0*/) { Init(rule, degree); }
...
bool ConnSpec::IsParam(std::string param_name) {
  if (param_name == "rule" || param_name == "indegree" ||
      param_name == "outdegree" || param_name == "total_num") {
    return true;
  } else {
    return false;
  }
...
SynSpec::SynSpec(float weight, float delay) { Init(weight, delay); }
...
int NESTGPU::Connect(inode_t i_source, inode_t n_source, inode_t i_target,
                     inode_t n_target, ConnSpec &conn_spec, SynSpec &syn_spec) {
  CheckUncalibrated("Connections cannot be created after calibration");

  return conn_->connect(i_source, n_source, i_target, n_target, conn_spec,
                        syn_spec);
}

int NESTGPU::Connect(inode_t i_source, inode_t n_source, inode_t *target,
                     inode_t n_target, ConnSpec &conn_spec, SynSpec &syn_spec) {
  CheckUncalibrated("Connections cannot be created after calibration");
  ...
  int ret = conn_->connect(i_source, n_source, d_target, n_target, conn_spec,
                           syn_spec);
...
int NESTGPU::Connect(inode_t *source, inode_t n_source, inode_t i_target,
                     inode_t n_target, ConnSpec &conn_spec, SynSpec &syn_spec) {
  CheckUncalibrated("Connections cannot be created after calibration");
  ...
  gpuErrchk(cudaMemcpy(d_source, source, n_source * sizeof(inode_t),
                       cudaMemcpyHostToDevice));
  int ret = conn_->connect(d_source, n_source, i_target, n_target, conn_spec,
                           syn_spec);
...
int NESTGPU::Connect(inode_t *source, inode_t n_source, inode_t *target,
                     inode_t n_target, ConnSpec &conn_spec, SynSpec &syn_spec) {
  CheckUncalibrated("Connections cannot be created after calibration");
  inode_t *d_source;
  CUDAMALLOCCTRL("&d_source", &d_source, n_source * sizeof(inode_t));
  ...
  gpuErrchk(cudaMemcpy(d_target, target, n_target * sizeof(inode_t),
                       cudaMemcpyHostToDevice));
  int ret = conn_->connect(d_source, n_source, d_target, n_target, conn_spec,
                           syn_spec);
  CUDAFREECTRL("d_source", d_source);
  CUDAFREECTRL("d_target", d_target);
...
int NESTGPU::Connect(NodeSeq source, NodeSeq target, ConnSpec &conn_spec,
                     SynSpec &syn_spec) {
...
int NESTGPU::Connect(NodeSeq source, std::vector<inode_t> target,
  ...
  return Connect(source.i0, source.n, target.data(), target.size(), conn_spec,
                 syn_spec);
}

int NESTGPU::Connect(std::vector<inode_t> source, NodeSeq target,
                     ConnSpec &conn_spec, SynSpec &syn_spec) {
  return Connect(source.data(), source.size(), target.i0, target.n, conn_spec,
                 syn_spec);
}

int NESTGPU::Connect(std::vector<inode_t> source, std::vector<inode_t> target,
                     ConnSpec &conn_spec, SynSpec &syn_spec) {
  return Connect(source.data(), source.size(), target.data(), target.size(),
                 conn_spec, syn_spec);
}

int NESTGPU::RemoteConnect(int i_source_host, inode_t i_source,
                           inode_t n_source, int i_target_host,
                           inode_t i_target, inode_t n_target, int i_host_group,
                           ConnSpec &conn_spec, SynSpec &syn_spec) {
  CheckUncalibrated("Connections cannot be created after calibration");

  return conn_->remoteConnect(i_source_host, i_source, n_source, i_target_host,
                              i_target, n_target, i_host_group, conn_spec,
                              syn_spec);
}

int NESTGPU::RemoteConnect(int i_source_host, inode_t i_source,
                           inode_t n_source, int i_target_host, inode_t *target,
                           inode_t n_target, int i_host_group,
                           ConnSpec &conn_spec, SynSpec &syn_spec) {
  CheckUncalibrated("Connections cannot be created after calibration");
  ...
  // cudaMemcpyHostToDevice ) );
  int ret =
      conn_->remoteConnect(i_source_host, i_source, n_source, i_target_host,
                           target, n_target, i_host_group, conn_spec, syn_spec);
  // CUDAFREECTRL( "d_target", d_target );

  return ret;
}

int NESTGPU::RemoteConnect(int i_source_host, inode_t *source, inode_t n_source,
                           int i_target_host, inode_t i_target,
                           inode_t n_target, int i_host_group,
                           ConnSpec &conn_spec, SynSpec &syn_spec) {
  CheckUncalibrated("Connections cannot be created after calibration");
  ...
  // gpuErrchk( cudaMemcpy( d_source, source, n_source * sizeof( inode_t ),
  // cudaMemcpyHostToDevice ) );
  int ret = conn_->remoteConnect(i_source_host, source, n_source, i_target_host,
                                 i_target, n_target, i_host_group, conn_spec,
                                 syn_spec);
  // CUDAFREECTRL( "d_source", d_source );
...
int NESTGPU::RemoteConnect(int i_source_host, inode_t *source, inode_t n_source,
  ...
  CUDAMALLOCCTRL( "&d_source", &d_source, n_source * sizeof( inode_t ) );
  gpuErrchk( cudaMemcpy( d_source, source, n_source * sizeof( inode_t ),
  cudaMemcpyHostToDevice ) ); inode_t* d_target; CUDAMALLOCCTRL( "&d_target",
  &d_target, n_target * sizeof( inode_t ) ); gpuErrchk( cudaMemcpy( d_target,
  target, n_target * sizeof( inode_t ), cudaMemcpyHostToDevice ) );
  */
  int ret =
      conn_->remoteConnect(i_source_host, source, n_source, i_target_host,
                           target, n_target, i_host_group, conn_spec, syn_spec);
  // CUDAFREECTRL( "d_source", d_source );
  // CUDAFREECTRL( "d_target", d_target );
...
int NESTGPU::RemoteConnect(int i_source_host, NodeSeq source, int i_target_host,
                           NodeSeq target, int i_host_group,
                           ConnSpec &conn_spec, SynSpec &syn_spec) {
  return RemoteConnect(i_source_host, source.i0, source.n, i_target_host,
                       target.i0, target.n, i_host_group, conn_spec, syn_spec);
}
...
int NESTGPU::RemoteConnect(int i_source_host, NodeSeq source, int i_target_host,
                           ...
                           ConnSpec &conn_spec, SynSpec &syn_spec) {
  return RemoteConnect(i_source_host, source.i0, source.n, i_target_host,
                       target.data(), target.size(), i_host_group, conn_spec,
                       syn_spec);
}
...
int NESTGPU::RemoteConnect(int i_source_host, std::vector<inode_t> source,
  ...
  return RemoteConnect(i_source_host, source.data(), source.size(),
                       i_target_host, target.i0, target.n, i_host_group,
                       conn_spec, syn_spec);
}
...
int NESTGPU::ConnectDistributedFixedIndegree(
    int *source_host_arr, int n_source_host, inode_t *source_arr,
    inode_t *n_source_arr, int *target_host_arr, int n_target_host,
    inode_t *target_arr, inode_t *n_target_arr, int indegree, int i_host_group,
    SynSpec &syn_spec) {
  CheckUncalibrated("Connections cannot be created after calibration");

  int ret = conn_->connectDistributedFixedIndegree(
      source_host_arr, n_source_host, source_arr, n_source_arr, target_host_arr,
      n_target_host, target_arr, n_target_arr, indegree, i_host_group,
      syn_spec);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// Build connections with fixed indegree rule for source neurons and target
// neurons distributed across MPI processes (hosts) Case with both source nodes
// and target nodes stored in arrays
///////////////////////////////////////////////////////////////////////////////////////////////////////
int NESTGPU::ConnectDistributedFixedIndegree(
    int *source_host_arr, int n_source_host, inode_t **source_arr,
    inode_t *n_source_arr, int *target_host_arr, int n_target_host,
