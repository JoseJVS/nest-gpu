...
int SynModel::SetParam(std::string param_name, float val) {
  ...
  float *d_param_pt = d_param_arr_ + i_param;
  gpuErrchk(
      cudaMemcpy(d_param_pt, &val, sizeof(float), cudaMemcpyHostToDevice));
  return 0;
}
...
int NESTGPU::SynGroupCalibrate() {
  ...
  if (n_group > 0) {
    ...
    CUDAMALLOCCTRL("&d_SynGroupTypeMap", &d_SynGroupTypeMap,
                   n_group * sizeof(int));
    CUDAMALLOCCTRL("&d_SynGroupParamMap", &d_SynGroupParamMap,
