...
__global__ void ArrayInit(int array_size, int n_var, int n_param, double *x_arr,
                          float *h_arr, float *y_arr, float *par_arr,
                          double x_min, float h, DataStruct data_struct) {
...
template <class DataStruct>
__global__ void ArrayCalibrate(int array_size, int n_var, int n_param,
                               double *x_arr, float *h_arr, float *y_arr,
                               float *par_arr, double x_min, float h,
...
__device__ void RK5Step(double &x, float *y, float &h, float h_min, float h_max,
  ...
  for (;;) {
    ...
    for (int i = 0; i < NVAR; i++) {
      y_new[i] = y[i] + h * (a61 * k1[i] + a62 * k2[i] + a63 * k3[i] +
                             a64 * k4[i] + a65 * k5[i]);
    }
    ...
    for (int i = 0; i < NVAR; i++) {
      y_new[i] =
          y[i] + h * (a71 * k1[i] + a73 * k3[i] + a74 * k4[i] + a76 * k6[i]);
    }
...
template <int NVAR, int NPARAM, class DataStruct>
__device__ void RK5Update(double &x, float *y, double x1, float &h, float h_min,
                          float *param, DataStruct data_struct) {
...
template <int NVAR, int NPARAM, class DataStruct>
__global__ void ArrayUpdate(int array_size, double *x_arr, float *h_arr,
                            float *y_arr, float *par_arr, double x1,
                            float h_min, DataStruct data_struct) {
...
template <class DataStruct> class RungeKutta5 {
  int array_size_;
  int n_var_;
  int n_param_;
...
template <class DataStruct> RungeKutta5<DataStruct>::~RungeKutta5() { Free(); }

template <class DataStruct> int RungeKutta5<DataStruct>::Free() {
...
int RungeKutta5<DataStruct>::Init(int array_size, int n_var, int n_param,
  ...
  CUDAMALLOCCTRL("&d_ParamArr", &d_ParamArr,
                 array_size_ * n_param_ * sizeof(float));

  ArrayInit<DataStruct><<<(array_size + 1023) / 1024, 1024>>>(
      array_size_, n_var, n_param, d_XArr, d_HArr, d_YArr, d_ParamArr, x_min, h,
      data_struct);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaDeviceSynchronize());
...
int RungeKutta5<DataStruct>::Calibrate(double x_min, float h,
  ...
  ArrayCalibrate<DataStruct><<<(array_size_ + 1023) / 1024, 1024>>>(
      array_size_, n_var_, n_param_, d_XArr, d_HArr, d_YArr, d_ParamArr, x_min,
      h, data_struct);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaDeviceSynchronize());
...
template <class DataStruct>
int RungeKutta5<DataStruct>::GetY(int i_var, int i_array, int n_elem,
                                  float *y) {
