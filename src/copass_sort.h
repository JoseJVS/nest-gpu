...
template <class KeyT, class ElementT, class KeyArrayT, class ArrayT,
          class AuxArrayT>
int sort_template(KeyArrayT key_array, ArrayT *h_subarray, uint k,
                  position_t block_size, void *d_storage, int64_t &st_bytes);

template <class ElementT, class ArrayT, class AuxArrayT>
int extract_partitions(ArrayT *d_subarray, uint k, uint k_next_pow_2,
...
int last_step(position_t *local_d_m_d, position_t *local_d_m_u,
              position_t *local_d_sum_m_d, position_t local_h_sum_m_d,
              position_t tot_part_size, uint k, uint kp_next_pow_2,
...
int last_step_case2(ArrayT *d_subarray, position_t tot_part_size, uint k,
                    position_t *d_part_size, position_t *d_m_d,
                    position_t *d_m_u, position_t h_sum_m_d, KeyT *d_extra_elem,
                    KeyT *h_extra_elem, int *d_extra_elem_idx,
                    int *h_extra_elem_idx, int *d_n_extra_elems);

////////////// Temporary for checking
template <class KeyT> KeyT *get_aux_array_keys();

template <class ValueT> ValueT *get_aux_array_values();

position_t *get_part_size();

template <class KeyT> int alloc(position_t n, position_t block_size);

template <class KeyT>
int sort(KeyT *d_keys, position_t n, position_t block_size, void *d_storage,
         int64_t &st_bytes);

template <class KeyT, class ValueT>
int sort(KeyT *d_keys, ValueT *d_values, position_t n, position_t block_size,
         void *d_storage, int64_t &st_bytes);

template <class KeyT>
int sort(KeyT **key_subarray, position_t n, position_t block_size,
         void *d_storage, int64_t &st_bytes);

template <class KeyT, class ValueT>
int sort(KeyT **key_subarray, ValueT **value_subarray, position_t n,
         position_t block_size, void *d_storage, int64_t &st_bytes);
...
int copass_sort::extract_partitions(ArrayT *d_subarray, uint k,
                                    uint k_next_pow_2, position_t *d_part_size,
                                    position_t *d_part_size_cumul,
                                    AuxArrayT d_aux_array) {
  prefix_scan<position_t, 1024>
      <<<1, 512>>>(d_part_size, d_part_size_cumul, k, k_next_pow_2);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaDeviceSynchronize());

  extract_partitions_kernel<ElementT, ArrayT, AuxArrayT>
      <<<k, 1024>>>(d_subarray, k, d_part_size, d_part_size_cumul, d_aux_array);
...
template <class KeyT, class ArrayT>
int copass_sort::last_step_case2(ArrayT *d_subarray, position_t tot_part_size,
                                 uint k, position_t *d_part_size,
                                 position_t *d_m_d, position_t *d_m_u,
                                 position_t h_sum_m_d, KeyT *d_extra_elem,
                                 KeyT *h_extra_elem, int *d_extra_elem_idx,
                                 int *h_extra_elem_idx, int *d_n_extra_elems) {
  ...
  if (tot_diff > 0) {
    case2_extra_elems_kernel<KeyT, ArrayT>
        <<<1, 1024>>>(d_subarray, k, d_m_d, d_m_u, d_extra_elem,
                      d_extra_elem_idx, d_n_extra_elems);
    ...
    gpuErrchk(cudaMemcpy(h_extra_elem, d_extra_elem,
                         n_extra_elems * sizeof(KeyT), cudaMemcpyDeviceToHost));
    ...
    gpuErrchk(cudaMemcpy(d_extra_elem_idx, h_extra_elem_idx,
                         n_extra_elems * sizeof(int), cudaMemcpyHostToDevice));

    /////////////////////////////////////////////////

    case2_inc_partitions_kernel<<<1, 1024>>>(d_part_size, d_extra_elem_idx,
                                             tot_diff);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
  }
...
template <class KeyT, class ElementT, class KeyArrayT, class ArrayT,
          class AuxArrayT>
int copass_sort::sort_template(KeyArrayT key_array, ArrayT *h_subarray, uint k,
                               position_t block_size, void *d_storage,
                               int64_t &st_bytes) {
  //////////////////////////////////////////////////////////////////////
  ...
  cudaReusableAlloc(d_storage, st_bytes, &d_part_size_cumul, (k + 1),
                    sizeof(position_t));
  ...
  gpuErrchk(cudaMemcpyAsync(d_subarray, h_subarray, k * sizeof(ArrayT),
  ...
  for (uint i_sub = 0; i_sub < k - 1; i_sub++) {
    ...
    threshold_range_kernel<KeyT, ArrayT, 1024>
        <<<1, k>>>(d_subarray, tot_part_size, k, d_t_u, d_t_d);

    // DBGCUDASYNC
    CUDASYNC
    search_multi_down<KeyT, ArrayT, 1024>(d_subarray, k, d_t_u, d_m_u,
                                          d_sum_m_u);
    CUDASYNC
    ...
    /////////////////////////////////////////////////////////////
    if (tot_part_size >= h_sum_m_u) { // m_u -> m_d
      search_multi_up<KeyT, ArrayT, 1024>(d_subarray, k, d_t_u, d_mu_u,
                                          d_sum_mu_u);
      /////////////////////
      ...
      last_step(d_m_u, d_mu_u, d_sum_m_u, h_sum_m_u, tot_part_size, k,
                kp_next_pow_2, d_part_size, d_diff, d_diff_cumul, h_diff,
                h_diff_cumul, d_num_down);
      if (print_gpu_cpu_vrb) {
        CUDASYNC
    ...
    //////////////////////////////////////////////////////////////
      ...
      search_multi_down<KeyT, ArrayT, 1024>(d_subarray, k, d_t_d, d_mu_d,
                                            d_sum_mu_d);
      /////////////////////
      ...
      last_step(d_mu_d, d_m_d, d_sum_mu_d, h_sum_mu_d, tot_part_size, k,
                kp_next_pow_2, d_part_size, d_diff, d_diff_cumul, h_diff,
                h_diff_cumul, d_num_down);
      if (print_gpu_cpu_vrb) {
        CUDASYNC
      ...
      for (;;) {
        max_diff_kernel<ArrayT, 1024>
            <<<1, 1024>>>(d_m_u, d_m_d, k, d_subarray, d_max_diff, d_arg_max);
        DBGCUDASYNC
        ...
        if (h_max_diff <= 1) {
          ...
          last_step_case2<KeyT, ArrayT>(
              d_subarray, tot_part_size, k, d_part_size, d_m_d, d_m_u,
              h_sum_m_d, d_extra_elem, h_extra_elem, d_extra_elem_idx,
              h_extra_elem_idx, d_n_extra_elems);
          if (print_gpu_cpu_vrb) {
            CUDASYNC
        ...
        eval_t_tilde_kernel<KeyT, ArrayT>
            <<<1, 1>>>(d_subarray, d_m_u, d_m_d, d_arg_max, d_t_tilde);
        DBGCUDASYNC

        search_multi_up<KeyT, ArrayT, 1024>(d_subarray, k, d_t_tilde, d_mu_u,
                                            d_sum_mu_u);
        search_multi_down<KeyT, ArrayT, 1024>(d_subarray, k, d_t_tilde, d_mu_d,
                                              d_sum_mu_d);
        ...
        if (tot_part_size < h_sum_mu_d) {
          ...
          last_step(d_mu_d, d_mu_u, d_sum_mu_d, h_sum_mu_d, tot_part_size, k,
                    kp_next_pow_2, d_part_size, d_diff, d_diff_cumul, h_diff,
                    h_diff_cumul, d_num_down);
          if (print_gpu_cpu_vrb) {
            CUDASYNC
    ...
    extract_partitions<ElementT, ArrayT>(d_subarray, k, k_next_pow_2,
                                         d_part_size, d_part_size_cumul,
                                         d_aux_array);
    ...
    CopyArray<ElementT, ArrayT, AuxArrayT>
        <<<(tot_part_size + 1023) / 1024, 1024>>>(target_array[i_sub],
                                                  d_aux_array, tot_part_size);
  }
...
int copass_sort::sort(KeyT *d_keys, position_t n, position_t block_size,
                      void *d_storage, int64_t &st_bytes, int64_t skip) {
  if (skip >= block_size) {
  ...
  for (uint i = 0; i < k; i++) {
    ...
    // the maximum storage requirement can be computed as the maximum between
    // the first and second block no matter the number of blocks
  ...
  sort_template<KeyT, KeyT, contiguous_array<KeyT>, contiguous_array<KeyT>,
                contiguous_array<KeyT>>(key_array, h_subarray, k, block_size,
                                        d_storage, st_bytes);
...
int copass_sort::sort(KeyT *d_keys, ValueT *d_values, position_t n,
  ...
  sort_template<KeyT, key_value<KeyT, ValueT>, contiguous_array<KeyT>,
...
int copass_sort::sort(KeyT **key_subarray, position_t n, position_t block_size,
  ...
  st_bytes = 0;
  uint k =
      (uint)((n + skip + block_size - 1) / block_size); // number of subarrays
  ...
  cudaReusableAlloc(d_storage, st_bytes, &d_key_array_data_pt, k,
                    sizeof(KeyT *));
  if (d_storage != NULL) {
    gpuErrchk(cudaMemcpy(d_key_array_data_pt, key_subarray, k * sizeof(KeyT *),
                         cudaMemcpyHostToDevice));
  }
...
int copass_sort::sort(KeyT **key_subarray, ValueT **value_subarray,
                      position_t n, position_t block_size, void *d_storage,
  ...
  cudaReusableAlloc(d_storage, st_bytes, &d_value_array_data_pt, k,
                    sizeof(ValueT *));
