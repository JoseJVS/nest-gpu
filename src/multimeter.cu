...
Record::Record(std::vector<BaseNeuron *> neur_vect, std::string file_name,
    ...
    : neuron_vect_(neur_vect), file_name_(file_name),
      var_name_vect_(var_name_vect), i_neuron_vect_(i_neur_vect),
...
int Record::WriteRecord(float t, long long time_idx) {
  float var;
...
int Multimeter::WriteRecords(float t, long long time_idx) {
  for (unsigned int i = 0; i < record_vect_.size(); i++) {
