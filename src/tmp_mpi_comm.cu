...
// Maybe the world group can be initialized by a specialized command in the Init
// that calls the CreateHostGroup method giving the array of all hosts' indexes
...
// however maybe in this case it is not necessary to explicitly store their
// indexes n_hosts[group_local_id] = host_group_[group_local_id].size();
std::vector<std::vector<int>> host_group_;

// Indexes of source nodes of each host that communicate spikes to a specific
// host group three-dimensional array, indexes: [group_local_id][i_host][i_node]
// besides the index group_local_id, the content should be the same across all
// hosts of the group
...
int CreateHostGroup(int *host_arr, int n_hosts) {
  ...
  if (this_host_is_in_group) {
    ...
    // if this host is not in the group, set the entry of host_group_local_id_
    // to -1
...
//              = 0 for the world group, which includes all hosts (i.e. all MPI
//              processes) > 0 for all the other host groups
template <class T>
...
int Calibrate() {
  ...
  for (uint group_local_id = 0; group_local_id < nhg; group_local_id++) {
    ...
    for (uint gi_host = 0; gi_host < nh; gi_host++) { // loop on hosts
      ...
      std::copy(host_group_source_node_[group_local_id][gi_host].begin(),
      ...
      if (src_host != this_host_) { // skip self host
        ...
        if (n_node_map > 0) {
          ...
          for (int ib = 0; ib < n_map_blocks; ib++) {
            ...
            CudaMemCpy(&hc_remote_source_node_map_[group_local_id][gi_host]
                                                  [ib * node_map_block_size_],
                       h_remote_source_node_map_[group_local_id][gi_host][ib],
                       n_elem);
            CudaMemCpy(&hc_image_node_map_[group_local_id][gi_host]
                                          [ib * node_map_block_size_],
          ...
          for (int i = 0; i < n_node_map; i++) {
            ...
            auto it = std::find(
                host_group_source_node_vect_[group_local_id][gi_host].begin(),
                host_group_source_node_vect_[group_local_id][gi_host].end(),
                src_node);
            if (it ==
                host_group_source_node_vect_[group_local_id][gi_host].end()) {
            ...
            inode_t pos =
                it -
                host_group_source_node_vect_[group_local_id][gi_host].begin();
            host_group_local_node_index_[pos] =
        ...
        for (uint i = 0; i < n_src; i++) {
          inode_t i_source =
              host_group_source_node_vect_[group_local_id][source_host][i];
