...
__global__ void GetSpikes(double *spike_array, int array_size, int n_port,
                          ...
                          int port_weight_arr_step, int port_weight_port_step,
                          float *port_input_arr, int port_input_arr_step,
                          int port_input_port_step) {
  ...
  if (i_target < array_size && port < n_port) {
    ...
    int port_input =
        i_target * port_input_arr_step + port_input_port_step * port;
    int port_weight =
        i_target * port_weight_arr_step + port_weight_port_step * port;
    double d_val = (double)port_input_arr[port_input] +
                   spike_array[i_array] * port_weight_arr[port_weight];
...
int NESTGPU::ClearGetSpikeArrays() {
  for (unsigned int i = 0; i < node_vect_.size(); i++) {
    ...
    if (bn->get_spike_array_ != nullptr) {
      gpuErrchk(cudaMemsetAsync(bn->get_spike_array_, 0,
                                bn->n_node_ * bn->n_port_ * sizeof(double)));
    }
  }
