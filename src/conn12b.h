...
template <>
__device__ __forceinline__ void setConnDelay<conn12b_key>(conn12b_key &conn_key,
                                                          int delay) {
...
setConnTarget<conn12b_struct>(conn12b_struct &conn, inode_t target) {
  conn.target_port_syn =
      (conn.target_port_syn & (~TargetMask)) | (target << MaxPortSynNBits);
}

template <>
__device__ __forceinline__ void
setConnPort<conn12b_key, conn12b_struct>(conn12b_key &conn_key,
                                         conn12b_struct &conn, int port) {
  conn.target_port_syn =
      (conn.target_port_syn & (~PortMask)) | (port << (MaxSynNBits + 1));
}

template <>
__device__ __forceinline__ void
setConnSyn<conn12b_key, conn12b_struct>(conn12b_key &conn_key,
                                        conn12b_struct &conn, int syn) {
...
template <>
__device__ __forceinline__ int
getConnPort<conn12b_key, conn12b_struct>(conn12b_key &conn_key,
                                         conn12b_struct &conn) {
...
template <>
__device__ __forceinline__ int
getConnSyn<conn12b_key, conn12b_struct>(conn12b_key &conn_key,
                                        conn12b_struct &conn) {
...
getConnRemoteFlag<conn12b_key, conn12b_struct>(conn12b_key &conn_key,
                                               conn12b_struct &conn) {
...
template <>
__device__ __forceinline__ void
clearConnRemoteFlag<conn12b_key, conn12b_struct>(conn12b_key &conn_key,
                                                 conn12b_struct &conn) {
  conn.target_port_syn = conn.target_port_syn & ~((uint)1 << MaxSynNBits);
}

template <>
int ConnectionTemplate<conn12b_key, conn12b_struct>::_setMaxNodeNBits(
    int max_node_nbits);

template <>
int ConnectionTemplate<conn12b_key, conn12b_struct>::_setMaxDelayNBits(
    int max_delay_nbits);

template <>
int ConnectionTemplate<conn12b_key, conn12b_struct>::_setMaxSynNBits(
    int max_syn_nbits);
