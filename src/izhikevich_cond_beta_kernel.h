...
enum ScalVarIndexes { i_V_m = 0, i_u, N_SCAL_VAR };
...
const std::string izhikevich_cond_beta_scal_param_name[N_SCAL_PARAM] = {
    "V_th", "a", "b", "c", "d", "I_e", "t_ref", "refractory_step", "den_delay"};

const std::string izhikevich_cond_beta_port_param_name[N_PORT_PARAM] = {
    "E_rev", "tau_rise", "tau_decay", "g0"};
...
template <> int izhikevich_cond_beta::UpdateNR<0>(long long it, double t1);
...
__device__ void ExternalUpdate(double x, float *y, float *param,
                               bool end_time_step,
                               izhikevich_cond_beta_rk5 data_struct) {
  izhikevich_cond_beta_ns::ExternalUpdate<NVAR, NPARAM>(
