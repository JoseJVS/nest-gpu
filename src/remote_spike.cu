...
// ExternalNodeTargetHostGroupId[ i_source ][ itg ] itg = 0, ...,
// NExternalNodeTargetHostGroup[i_source]-1
__device__ uint *
    *ExternalNodeTargetHostGroupId; // [n_local_nodes ][num. of target host
                                    // groups of the node ]
uint **d_ExternalNodeTargetHostGroupId;

// positions of local source nodes in host group node map
// ExternalHostGroupNodeId[ i_source ][ itg ] =
// host_group_local_source_node_map[group_local_id][i_source]
__device__ uint **ExternalHostGroupNodeId; // [n_local_nodes ][num. of target
                                           // host groups of the node ]
uint **d_ExternalHostGroupNodeId;
...
// index of the first spike of the spike subarray that must be sent to the host
// group group_local_id (the spikes to be sent to all host groups are flattened
// on a single one-dimensional array) ExternalHostGroupSpikeIdx0[ group_local_id
// ]
__device__ uint *ExternalHostGroupSpikeIdx0; // [ num. of local host groups ]
uint *d_ExternalHostGroupSpikeIdx0;
std::vector<uint> h_ExternalHostGroupSpikeIdx0;

// flattened one-dimensional array of spikes that must be sent to all host
// groups
__device__ uint *ExternalHostGroupSpikeNodeId; // [MaxSpikeNum];
uint *d_ExternalHostGroupSpikeNodeId;
std::vector<uint> h_ExternalHostGroupSpikeNodeId;

// flattened one-dimensional array of multiplicity of spikes that must be sent
// to all host groups
...
__global__ void countExternalSpikesPerTargetHostGroup() {
  const uint i_spike = blockIdx.x;
  if (i_spike < *ExternalSpikeNum) {
    // printf("ExternalSpikeNum: %d\ti_spike: %d\n", *ExternalSpikeNum,
    // i_spike);
...
__global__ void organizeExternalSpikesPerTargetHostGroup() {
  const uint i_spike = blockIdx.x;
  if (i_spike < *ExternalSpikeNum) {
    // printf("ExternalSpikeNum: %d\ti_spike: %d\n", *ExternalSpikeNum,
    //	   i_spike);
...
int NESTGPU::ExternalSpikeReset() {
  ...
  gpuErrchk(cudaMemset(d_ExternalHostGroupSpikeNum, 0,
                       conn_->getHostGroup().size() * sizeof(uint)));
  std::fill(h_ExternalHostGroupSpikeNum.begin(),
            h_ExternalHostGroupSpikeNum.end(), 0);
...
int NESTGPU::ExternalSpikeInit() {
  ...
  for (int ih = 1; ih < n_hosts_; ih++) {
    h_ExternalSourceSpikeDispl[ih] =
        h_ExternalSourceSpikeDispl[ih - 1] + max_spike_per_host_;
  }
  ...
  CUDAMALLOCCTRL("&d_ExternalSpikeSourceNode", &d_ExternalSpikeSourceNode,
                 max_spike_per_host_ * sizeof(uint));

  if (remote_spike_mul_) {
    h_ExternalSpikeMul.resize(max_spike_per_host_);
    CUDAMALLOCCTRL("&d_ExternalSpikeMul", &d_ExternalSpikeMul,
                   max_spike_per_host_ * sizeof(float));
    CUDAMALLOCCTRL("&d_ExternalTargetSpikeMul", &d_ExternalTargetSpikeMul,
                   max_remote_spike_num_ * sizeof(float));
    CUDAMALLOCCTRL("&d_ExternalSourceSpikeMul", &d_ExternalSourceSpikeMul,
                   max_remote_spike_num_ * sizeof(float));
    CUDAMALLOCCTRL("&d_ExternalHostGroupSpikeMul", &d_ExternalHostGroupSpikeMul,
                   max_remote_spike_num_ * sizeof(float));
  }
  ...
  CUDAMALLOCCTRL("&d_ExternalTargetSpikeNodeId", &d_ExternalTargetSpikeNodeId,
                 max_remote_spike_num_ * sizeof(uint));
  CUDAMALLOCCTRL("&d_ExternalHostGroupSpikeNodeId",
  ...
  CUDAMALLOCCTRL("&d_ExternalHostGroupSpikeIdx0", &d_ExternalHostGroupSpikeIdx0,
                 (nhg + 1) * sizeof(uint));
  CUDAMALLOCCTRL("&d_ExternalHostGroupSpikeNum", &d_ExternalHostGroupSpikeNum,
                 nhg * sizeof(uint));

  const std::vector<std::vector<int>> &node_target_host_group =
      conn_->getNodeTargetHostGroup();
  std::vector<std::vector<uint>> &host_group_local_source_node_map =
      conn_->getHostGroupLocalSourceNodeMap();
  ...
  if (ntg_tot > 0) {
    CUDAMALLOCCTRL("&hd_ExternalNodeTargetHostGroupId[0]",
                   &hd_ExternalNodeTargetHostGroupId[0],
                   ntg_tot * sizeof(uint));
    CUDAMALLOCCTRL("&hd_ExternalHostGroupNodeId[0]",
                   &hd_ExternalHostGroupNodeId[0], ntg_tot * sizeof(uint));
  } else {
  ...
  for (inode_t i_node = 0; i_node < n_node; i_node++) {
    ...
    if (node_target_host_group[i_node].size() != ntg) {
      throw ngpu_exception(
          std::string("Inconsistent number of target host groups for node ") +
          std::to_string(i_node) + "\n\tn_node_target_host_group[i_node] = " +
          std::to_string(ntg) + "\n\tnode_target_host_group[i_node].size() = " +
          std::to_string(node_target_host_group[i_node].size()));
    }
    if (ntg > 0) {
      std::copy(node_target_host_group[i_node].begin(),
                node_target_host_group[i_node].end(),
                node_target_host_group_it);
      for (uint itg = 0; itg < ntg; itg++) {
        uint group_local_id = node_target_host_group_it[itg];
        inode_t node_pos =
            host_group_local_source_node_map[group_local_id][i_node];
    ...
    if (i_node < n_node - 1) {
      if (ntg_tot > 0) {
        hd_ExternalNodeTargetHostGroupId[i_node + 1] =
            hd_ExternalNodeTargetHostGroupId[i_node] + ntg;
        hd_ExternalHostGroupNodeId[i_node + 1] =
            hd_ExternalHostGroupNodeId[i_node] + ntg;
      } else {
  ...
  if (ntg_tot > 0) {
    gpuErrchk(cudaMemcpy(hd_ExternalNodeTargetHostGroupId[0],
                         &node_target_host_group_flat[0],
                         ntg_tot * sizeof(uint), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(hd_ExternalHostGroupNodeId[0],
                         &host_group_node_id_flat[0], ntg_tot * sizeof(uint),
                         cudaMemcpyHostToDevice));
  }

  CUDAMALLOCCTRL("&d_ExternalNodeTargetHostGroupId",
                 &d_ExternalNodeTargetHostGroupId, n_node * sizeof(uint *));
  gpuErrchk(cudaMemcpy(d_ExternalNodeTargetHostGroupId,
                       &hd_ExternalNodeTargetHostGroupId[0],
                       n_node * sizeof(uint *), cudaMemcpyHostToDevice));
  CUDAMALLOCCTRL("&d_ExternalHostGroupNodeId", &d_ExternalHostGroupNodeId,
                 n_node * sizeof(uint *));
  gpuErrchk(cudaMemcpy(d_ExternalHostGroupNodeId,
                       &hd_ExternalHostGroupNodeId[0], n_node * sizeof(uint *),
                       cudaMemcpyHostToDevice));
  CUDAMALLOCCTRL("&d_NExternalNodeTargetHostGroup",
                 &d_NExternalNodeTargetHostGroup, n_node * sizeof(uint));
  gpuErrchk(cudaMemcpy(d_NExternalNodeTargetHostGroup,
                       &n_node_target_host_group[0], n_node * sizeof(uint),
                       cudaMemcpyHostToDevice));

  if (remote_spike_mul_) {
    DeviceExternalSpikeInit<<<1, 1>>>(
        n_hosts_, max_spike_per_host_, d_ExternalSpikeNum,
        d_ExternalSpikeSourceNode, d_ExternalSpikeMul, d_ExternalTargetSpikeNum,
        d_ExternalTargetSpikeIdx0, d_ExternalTargetSpikeNodeId,
        d_ExternalTargetSpikeMul, conn_->getDevNTargetHosts(),
        conn_->getDevNodeTargetHosts(), conn_->getDevNodeTargetHostIMap(),
        d_NExternalNodeTargetHostGroup, d_ExternalNodeTargetHostGroupId,
        d_ExternalHostGroupNodeId, d_ExternalHostGroupSpikeNum,
        d_ExternalHostGroupSpikeIdx0, d_ExternalHostGroupSpikeNodeId,
        d_ExternalHostGroupSpikeMul);

  } else {
    DeviceExternalSpikeInit<<<1, 1>>>(
        n_hosts_, max_spike_per_host_, d_ExternalSpikeNum,
        d_ExternalSpikeSourceNode, d_ExternalTargetSpikeNum,
        d_ExternalTargetSpikeIdx0, d_ExternalTargetSpikeNodeId,
        conn_->getDevNTargetHosts(), conn_->getDevNodeTargetHosts(),
        conn_->getDevNodeTargetHostIMap(), d_NExternalNodeTargetHostGroup,
        d_ExternalNodeTargetHostGroupId, d_ExternalHostGroupNodeId,
        d_ExternalHostGroupSpikeNum, d_ExternalHostGroupSpikeIdx0,
        d_ExternalHostGroupSpikeNodeId);
  }
  ExternalSpikeReset();
...
__global__ void DeviceExternalSpikeInit(
    uint n_hosts, uint max_spike_per_host, uint *ext_spike_num,
    uint *ext_spike_source_node, float *ext_spike_mul,
    uint *ext_target_spike_num, uint *ext_target_spike_idx0,
    uint *ext_target_spike_node_id, float *ext_target_spike_mul,
    uint *n_ext_node_target_host, uint **ext_node_target_host_id,
    uint **ext_node_id, uint *n_ext_node_target_host_group,
    uint **ext_node_target_host_group_id, uint **ext_host_group_node_id,
    uint *ext_host_group_spike_num, uint *ext_host_group_spike_idx0,
    uint *ext_host_group_spike_node_id, float *ext_host_group_spike_mul)
...
__global__ void DeviceExternalSpikeInit(
    uint n_hosts, uint max_spike_per_host, uint *ext_spike_num,
    uint *ext_spike_source_node, uint *ext_target_spike_num,
    uint *ext_target_spike_idx0, uint *ext_target_spike_node_id,
    uint *n_ext_node_target_host, uint **ext_node_target_host_id,
    uint **ext_node_id, uint *n_ext_node_target_host_group,
    uint **ext_node_target_host_group_id, uint **ext_host_group_node_id,
    uint *ext_host_group_spike_num, uint *ext_host_group_spike_idx0,
...
int NESTGPU::organizeExternalSpikes(int n_ext_spikes) {
  ...
  CUDASYNC;
  prefix_scan((int *)d_ExternalTargetSpikeIdx0, (int *)d_ExternalTargetSpikeNum,
              n_hosts_ + 1, true);
  DBGCUDASYNC;
  gpuErrchk(cudaMemset(d_ExternalTargetSpikeNum, 0, n_hosts_ * sizeof(uint)));
  ...
  // probably missing some condition checking if host groups and/or
  // point-to-point MPI communication are actually used
  countExternalSpikesPerTargetHostGroup<<<n_ext_spikes, 1024>>>();
  CUDASYNC;
  gpuErrchk(cudaMemcpy(
      &h_ExternalHostGroupSpikeNum[0], d_ExternalHostGroupSpikeNum,
      conn_->getHostGroup().size() * sizeof(uint), cudaMemcpyDeviceToHost));
  prefix_scan((int *)d_ExternalHostGroupSpikeIdx0,
              (int *)d_ExternalHostGroupSpikeNum,
              conn_->getHostGroup().size() + 1, true);
  // DBGCUDASYNC;
  CUDASYNC;
  gpuErrchk(cudaMemcpy(&h_ExternalHostGroupSpikeIdx0[0],
                       d_ExternalHostGroupSpikeIdx0,
                       (conn_->getHostGroup().size() + 1) * sizeof(uint),
...
int NESTGPU::CopySpikeFromRemote() {
  std::vector<std::vector<std::vector<int64_t>>> &host_group_local_node_index =
      conn_->getHostGroupLocalNodeIndex();

  // boolean flag activated if first connection of each image node is stored in
  // GPU memory
  ...
  const std::vector<int64_t> &h_first_out_connection =
      conn_->getFirstOutConnection();

  // vector of number of connections outgoing from each image node
  // [n_image_node]
  ...
  std::vector<int64_t> &h_spike_first_connection =
      conn_->getSpikeFirstConnection();
  ...
  const std::vector<std::vector<std::vector<uint>>> &hc_image_node_map =
      conn_->getHCImageNodeMap();
  ...
  for (int i_host = 0; i_host < n_hosts_; i_host++) {
    ...
    for (int i_spike = 0; i_spike < n_spike; i_spike++) {
      // pack spikes received from remote hosts
      inode_t node_pos =
          h_ExternalSourceSpikeNodeId[0]
                                     [i_host * max_spike_per_host_ + i_spike];
      if (first_out_conn_in_device_) {
        ...
        h_spike_first_connection[n_spike_tot] =
            h_first_out_connection[node_local - n_local_nodes];
        h_spike_n_connections[n_spike_tot] =
            h_n_out_connections[node_local - n_local_nodes];
      }
      n_spike_tot++;
      if (n_spike_tot >= max_remote_spike_num_) {
        throw ngpu_exception(
            std::string("Number of spikes received remotely ") +
            std::to_string(n_spike_tot) + " larger than limit " +
            std::to_string(max_remote_spike_num_));
      }
    }
  }
    ...
    for (uint gi_host = 0; gi_host < nh; gi_host++) {
      ...
      if (i_host != this_host_) {
        ...
        for (int i_spike = 0; i_spike < n_spike; i_spike++) {
          // pack spikes received from remote hosts
          inode_t node_pos =
              h_ExternalSourceSpikeNodeId[group_local_id]
                                         [gi_host * max_spike_per_host_ +
                                          i_spike];
          int64_t node_local =
              host_group_local_node_index[group_local_id][gi_host][node_pos];
          if (node_local >= 0) {
  ...
  if (n_spike_tot > 0) {
    if (first_out_conn_in_device) {
      ...
      gpuErrchk(cudaMemcpyAsync(
          d_ExternalSourceSpikeIdx0, &h_ExternalSourceSpikeIdx0[0],
          (n_hosts_ + 1) * sizeof(uint), cudaMemcpyHostToDevice));
      DBGCUDASYNC;
      // copy to GPU memory packed spikes from remote hosts
      gpuErrchk(cudaMemcpyAsync(
          d_ExternalSourceSpikeNodeId, &h_ExternalSourceSpikeNodeId_flat[0],
          n_spike_tot * sizeof(uint), cudaMemcpyHostToDevice));
      DBGCUDASYNC;
      ...
      std::vector<uint *> &d_n_remote_source_node_map =
          conn_->getDevNRemoteSourceNodeMap();
      ;
      ...
      checkMapIndexToImageNodeKernel<<<n_hosts_, 1024>>>(
          ...
          n_map, max_remote_spike_num_,
          this_host_); // n_spike_tot -> max_remote_spike_num_
      CUDASYNC;
#endif
