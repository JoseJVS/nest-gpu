...
int organizeDirectConnections(Connection *conn) {
  conn_ = conn;
  return conn->organizeDirectConnections(d_poiss_key_array_data_pt,
                                         d_poiss_subarray, d_poiss_num,
                                         d_poiss_sum, d_poiss_thresh);
}
}; // namespace poiss_conn
...
__global__ void PoissGenUpdateKernel(long long time_idx, int n_node,
                                     int max_delay, float *param_arr,
                                     int n_param, float *mu_arr) {
...
int poiss_gen::Init(int i_node_0, int n_node, int /*n_port*/, int i_group) {
...
int poiss_gen::buildDirectConnections() {
  ...
  return poiss_conn::conn_->buildDirectConnections(
      i_node_0_, n_node_, i_conn0_, n_dir_conn_, max_delay_, d_mu_arr_,
      d_poiss_key_array_);
}

int poiss_gen::SendDirectSpikes(long long time_idx) {
  return poiss_conn::conn_->sendDirectSpikes(
      time_idx, i_conn0_, n_dir_conn_, n_node_, max_delay_, d_mu_arr_,
      d_poiss_key_array_, d_curand_state_);
}

int poiss_gen::Calibrate(double, float) {
  // if the device is not connected to any other node, the calibration continues
