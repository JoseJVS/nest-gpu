...
enum PortParamIndexes { i_E_rev = 0, i_tau_syn, i_g0, N_PORT_PARAM };
...
const std::string user_m2_port_param_name[N_PORT_PARAM] = {"E_rev", "tau_syn",
                                                           "g0"};

const std::string user_m2_group_param_name[N_GROUP_PARAM] = {"h_min_rel",
                                                             "h0_rel"};
...
__device__ void ExternalUpdate(double x, float *y, float *param,
                               bool end_time_step, user_m2_rk5 data_struct) {
...
template <> int user_m2::UpdateNR<0>(long long it, double t1);

template <int N_PORT> int user_m2::UpdateNR(long long it, double t1) {
  if (N_PORT == n_port_) {
    const int NVAR = user_m2_ns::N_SCAL_VAR + user_m2_ns::N_PORT_VAR * N_PORT;
    const int NPARAM =
        user_m2_ns::N_SCAL_PARAM + user_m2_ns::N_PORT_PARAM * N_PORT;
...
__device__ void ExternalUpdate(double x, float *y, float *param,
                               bool end_time_step, user_m2_rk5 data_struct) {
  user_m2_ns::ExternalUpdate<NVAR, NPARAM>(x, y, param, end_time_step,
                                           data_struct);
}
