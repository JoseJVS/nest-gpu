...
__global__ void izhikevich_psc_exp_5s_Update(int n_node, int i_node_0,
                                             float *var_arr, float *param_arr,
                                             int n_var, int n_param, float V_th,
  ...
  if (i_neuron < n_node) {
    ...
    if (refractory_step > 0.0) {
      ...
      for (int i = 0; i < INTEGR_STEPS; i++) {
        ...
        V_m += h * (0.04 * v_old * v_old + 5.0 * v_old + 140.0 - u_old + I_syn +
                    I_e);
        u += h * a * (b * v_old - u_old);
        // exponential decaying PSC
        I_syn *= C_syn;
...
int izhikevich_psc_exp_5s::Update(long long it, double t1) {
  ...
  izhikevich_psc_exp_5s_Update<<<(n_node_ + 1023) / 1024, 1024>>>(
      ...
      c_, d_, n_refractory_steps, h, C_syn);
