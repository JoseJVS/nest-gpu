...
const std::string spike_gen_array_param_name[N_SPIKE_GEN_ARRAY_PARAM] = {
    "spike_times", "spike_gen_mul"};

__global__ void spike_generatorUpdate(int i_node_0, int n_node,
                                      long long i_time, int *n_spikes,
                                      int *i_spike, int **spike_time_idx,
                                      float **spike_gen_mul) {
...
int spike_generator::Init(int i_node_0, int n_node, int /*n_port*/,
  ...
  CUDAMALLOCCTRL("&d_spike_time_idx_", &d_spike_time_idx_,
                 n_node_ * sizeof(int *));
  CUDAMALLOCCTRL("&d_spike_gen_mul_", &d_spike_gen_mul_,
                 n_node_ * sizeof(float *));
  ...
  for (int in = 0; in < n_node_; in++) {
    ...
    if (n_spikes > 0) {
      if (spike_mul_vect_[in].size() == 0) {
        ...
        throw ngpu_exception("spike time array and spike multiplicity array "
                             "must have the same size in spike generator");
      }
      SetSpikes(in, n_spikes, spike_time_vect_[in].data(),
                spike_mul_vect_[in].data(), (float)time_min, time_resolution);
    }
  }
...
int spike_generator::SetSpikes(int irel_node, int n_spikes, float *spike_time,
                               float *spike_gen_mul, float time_min,
  ...
  CUDAMALLOCCTRL("&h_spike_time_idx_[irel_node]", &h_spike_time_idx_[irel_node],
                 n_spikes * sizeof(int));
  CUDAMALLOCCTRL("&h_spike_gen_mul_[irel_node]", &h_spike_gen_mul_[irel_node],
                 n_spikes * sizeof(float));

  cudaMemcpy(&d_spike_time_idx_[irel_node], &h_spike_time_idx_[irel_node],
             sizeof(int *), cudaMemcpyHostToDevice);
  cudaMemcpy(&d_spike_gen_mul_[irel_node], &h_spike_gen_mul_[irel_node],
             sizeof(float *), cudaMemcpyHostToDevice);

  int *spike_time_idx = new int[n_spikes];
  for (int i = 0; i < n_spikes; i++) {
    spike_time_idx[i] =
        (int)round((spike_time[i] - time_min) / time_resolution);
  ...
  cudaMemcpy(h_spike_time_idx_[irel_node], spike_time_idx,
             n_spikes * sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(h_spike_gen_mul_[irel_node], spike_gen_mul,
