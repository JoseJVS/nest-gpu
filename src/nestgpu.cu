...
enum ConnStructType { i_conn12b, i_conn16b, N_CONN_STRUCT_TYPE };

const std::string kernel_float_param_name[N_KERNEL_FLOAT_PARAM] = {
    "time_resolution",         "max_spike_num_fact",
    "max_spike_per_host_fact", "max_remote_spike_num_fact",
    "min_allowed_delay",       "use_all_source_node_fact"};

const std::string kernel_int_param_name[N_KERNEL_INT_PARAM] = {
    "rnd_seed",         "verbosity_level",  "max_spike_buffer_size",
    "max_node_n_bits",  "max_syn_n_bits",   "max_delay_n_bits",
    "conn_struct_type", "spike_buffer_algo"};
...
NESTGPU::NESTGPU() {
  ...
  CURAND_CALL(curandSetPseudoRandomGeneratorSeed(*random_generator_,
                                                 kernel_seed_ + this_host_));
...
uint NESTGPU::GetNLocalNodes() { return node_group_map_.size(); }

uint NESTGPU::GetNTotalNodes() {
  return GetNLocalNodes() + conn_->getNImageNodes();
}

uint NESTGPU::CheckImageNodes(uint n_nodes) {
...
int NESTGPU::setConnStructType(int conn_struct_type) {
  ...
  gpuErrchk(cudaMemcpyToSymbolAsync(input_spike_buffer_ns::have_n_out_conn_,
                                    &have_n_out_conn_, sizeof(bool)));
...
uint NESTGPU::CreateNodeGroup(uint n_nodes, int n_ports) {
  uint i_node_0 = GetNLocalNodes();
  ...
  if (n_ports > max_n_ports && max_n_ports_warning_) {
    std::cout
        << "Warning: node group number of receptor ports larger than maximum "
        << max_n_ports << "\n";
    std::cout << "\tConnections can be made only to ports with index smaller "
                 "than the maximum\n";
    std::cout << "\tunless the number of bits reserved to represent the port "
                 "is increased.\n";
    std::cout << "\tThis warning message can be suppressed by setting the "
                 "kernel parameter max_n_ports_warning to false"
              << "\n";
  }
...
int NESTGPU::Calibrate() {
  ...
  gpuErrchk(cudaMemcpyToSymbol(have_remote_spike_mul, &remote_spike_mul_,
                               sizeof(bool)));
  ///////////////////////////////////
  ...
  max_spike_num_ =
      (int)round(max_spike_num_fact_ * GetNTotalNodes() * max_delay_num);
  max_spike_num_ = (max_spike_num_ > 1) ? max_spike_num_ : 1;

  max_spike_per_host_ =
      (int)round(max_spike_per_host_fact_ * GetNLocalNodes() * max_delay_num);
  max_spike_per_host_ = (max_spike_per_host_ > 1) ? max_spike_per_host_ : 1;

  max_remote_spike_num_ =
      max_spike_per_host_ * n_hosts_ * max_remote_spike_num_fact_;
  max_remote_spike_num_ =
      (max_remote_spike_num_ > 1) ? max_remote_spike_num_ : 1;
  ...
  spikeBufferInit(GetNLocalNodes(), n_image_nodes, max_spike_buffer_size_,
                  conn_->getSpikeBufferAlgo());
...
int NESTGPU::PrintTimers(int verbosity_level) {
  ...
  if (verbosity_level >= 3) {
    std::cout << "\n";
    std::cout << HostIdStr()
              << "  SpikeBufferUpdate_time: " << SpikeBufferUpdate_time_
              << "\n";
    std::cout << HostIdStr()
              << "  poisson_generator_time: " << poisson_generator_time_
              << "\n";
    std::cout << HostIdStr() << "  neuron_Update_time: " << neuron_Update_time_
              << "\n";
    std::cout << HostIdStr()
              << "  copy_ext_spike_time: " << copy_ext_spike_time_ << "\n";
    std::cout << HostIdStr()
              << "  organizeExternalSpike_time: " << organizeExternalSpike_time_
              << "\n";
    std::cout << HostIdStr()
              << "  SendSpikeToRemote_time: " << SendSpikeToRemote_time_
              << "\n";
    std::cout << HostIdStr()
              << "  RecvSpikeFromRemote_time: " << RecvSpikeFromRemote_time_
              << "\n";
    std::cout << HostIdStr()
              << "  CopySpikeFromRemote_time: " << CopySpikeFromRemote_time_
              << "\n";
    std::cout << HostIdStr() << "  DeliverSpikesToInputBuffers_time: "
              << DeliverSpikesToInputBuffers_time_ << "\n";
    ...
    std::cout << HostIdStr() << "  SpikeReset_time: " << SpikeReset_time_
              << "\n";
    std::cout << HostIdStr()
              << "  ExternalSpikeReset_time: " << ExternalSpikeReset_time_
              << "\n";
    std::cout << HostIdStr() << "  CUDA_global_memory_allocation_time: "
              << cuda_error_ns::alloc_time_ << "\n";
    std::cout << HostIdStr() << "  CUDA_global_memory_deallocation_time: "
              << cuda_error_ns::free_time_ << "\n";
  }
  ...
  if (n_hosts_ > 1 && verbosity_level >= 4) {
    ...
    if (mpi_bitpack_) {
      ...
      std::cout << HostIdStr()
                << "  SpikeNumAllgather_send_: " << SpikeNumAllgather_send_
                << "\n";
      std::cout << HostIdStr() << "  SpikeNumAllgather_send_packed_: "
                << SpikeNumAllgather_send_packed_ << "\n";
      std::cout << HostIdStr()
                << "  SpikeNumAllgather_recv_: " << SpikeNumAllgather_recv_
                << "\n";
      std::cout << HostIdStr() << "  SpikeNumAllgather_recv_packed_: "
                << SpikeNumAllgather_recv_packed_ << "\n";
    }
  }
  if (n_hosts_ > 1 && verbosity_level >= 5) {
    std::cout << HostIdStr() << "  InsertHostGroupSourceNode_time: "
              << conn_->InsertHostGroupSourceNode_time_ << "\n";
    std::cout << HostIdStr() << "  ConnectRemoteConnectSource_time: "
              << conn_->ConnectRemoteConnectSource_time_ << "\n";
    std::cout << HostIdStr() << "  ConnectRemoteConnectTarget_time: "
              << conn_->ConnectRemoteConnectTarget_time_ << "\n";
    std::cout << HostIdStr() << "  SetUsedSourceNodes_time: "
              << conn_->SetUsedSourceNodes_time_ << "\n";
    std::cout << HostIdStr() << "  CountUsedSourceNodes_time: "
              << conn_->CountUsedSourceNodes_time_ << "\n";
    std::cout << HostIdStr() << "  AllocUsedSourceNodes_time: "
              << conn_->AllocUsedSourceNodes_time_ << "\n";
    std::cout << HostIdStr() << "  GetUsedSourceNodeIndex_time: "
              << conn_->GetUsedSourceNodeIndex_time_ << "\n";
    std::cout << HostIdStr() << "  SortUsedSourceNodeIndex_time: "
              << conn_->SortUsedSourceNodeIndex_time_ << "\n";
    std::cout << HostIdStr()
              << "  AllocNodeToMap_time: " << conn_->AllocNodeToMap_time_
              << "\n";
    std::cout << HostIdStr() << "  SearchNodeIndexNotInMap_time: "
              << conn_->SearchNodeIndexNotInMap_time_ << "\n";
    std::cout << HostIdStr() << "  AllocLocalSourceNodeMapBlocks_time: "
              << conn_->AllocLocalSourceNodeMapBlocks_time_ << "\n";
    std::cout << HostIdStr() << "  AllocRemoteSourceNodeMapBlocks_time: "
              << conn_->AllocRemoteSourceNodeMapBlocks_time_ << "\n";
    std::cout << HostIdStr()
              << "  InsertNodesInMap_time: " << conn_->InsertNodesInMap_time_
              << "\n";
    std::cout << HostIdStr() << "  SortSourceImageNodeMap_time: "
              << conn_->SortSourceImageNodeMap_time_ << "\n";
    std::cout << HostIdStr()
              << "  SetLocalNodeIndex_time: " << conn_->SetLocalNodeIndex_time_
              << "\n";
    std::cout << HostIdStr() << "  FixConnectionSourceNodeIndexes_time: "
              << conn_->FixConnectionSourceNodeIndexes_time_ << "\n";
    std::cout << HostIdStr() << "  SearchSourceNodesRangeInMap_time: "
              << conn_->SearchSourceNodesRangeInMap_time_ << "\n";
    std::cout << HostIdStr() << "  TranslateSourceNodeMap_time: "
              << conn_->TranslateSourceNodeMap_time_ << "\n";
    std::cout << HostIdStr() << "  MapSourceNodeSequence_time: "
              << conn_->MapSourceNodeSequence_time_ << "\n";
    std::cout << HostIdStr() << "  RemoteConnectTarget_time: "
              << conn_->RemoteConnectTarget_time_ << "\n";
    std::cout << HostIdStr() << "  RemoteConnectSource_time: "
              << conn_->RemoteConnectSource_time_ << "\n";
  }

  if (verbosity_level >= 1) {
    std::cout << HostIdStr()
              << "Building time: " << (build_real_time_ - start_real_time_)
              << "\n";
    std::cout << HostIdStr()
              << "Simulation time: " << (end_real_time_ - build_real_time_)
              << "\n";
  }
    ...
    if (node_vect_[i]->n_port_ > 0) {
      ...
      if (conn_->getSpikeBufferAlgo() == INPUT_SPIKE_BUFFER_ALGO) {
        input_spike_buffer_ns::
            GetInputSpikes<<<grid_dim, 1024>>> // block_dim>>>
            (node_vect_[i]->i_node_0_, node_vect_[i]->n_node_,
...
int NESTGPU::SetNeuronParam(int *i_node, int n_node, std::string param_name,
  ...
  std::vector<int> nodes = GetNodeArrayWithOffset(i_node, n_node, i_group);
  if (node_vect_[i_group]->IsPortParam(param_name)) {
    return node_vect_[i_group]->SetPortParam(nodes.data(), n_node, param_name,
                                             param, array_size);
  } else {
    return node_vect_[i_group]->SetArrayParam(nodes.data(), n_node, param_name,
                                              param, array_size);
  }
}
...
int NESTGPU::SetNeuronPtScalVarDistr(int *i_node, int n_node,
  ...
  int i_group;
  std::vector<int> nodes = GetNodeArrayWithOffset(i_node, n_node, i_group);
  return node_vect_[i_group]->SetScalVarDistr(nodes.data(), n_node, var_name,
                                              distribution_);
}
...
int NESTGPU::SetNeuronPtPortVarDistr(int *i_node, int n_node,
  ...
  int i_group;
  std::vector<int> nodes = GetNodeArrayWithOffset(i_node, n_node, i_group);
  return node_vect_[i_group]->SetPortVarDistr(nodes.data(), n_node, var_name,
                                              distribution_);
}
...
int NESTGPU::SetDistributionVectParam(std::string param_name, float val,
                                      int i) {
...
int NESTGPU::SetNeuronVar(int *i_node, int n_node, std::string var_name,
  ...
  std::vector<int> nodes = GetNodeArrayWithOffset(i_node, n_node, i_group);
  if (node_vect_[i_group]->IsPortVar(var_name)) {
    return node_vect_[i_group]->SetPortVar(nodes.data(), n_node, var_name, var,
                                           array_size);
  } else {
    return node_vect_[i_group]->SetArrayVar(nodes.data(), n_node, var_name, var,
                                            array_size);
  }
}
...
size_t NESTGPU::getCUDAMemHostUsed() { return cuda_error_ns::mem_used_; }

size_t NESTGPU::getCUDAMemHostPeak() { return cuda_error_ns::mem_max_; }
...
float *NESTGPU::RandomLognormalClipped(size_t n, float mean, float stddev,
  ...
  for (size_t i = 0; i < n; i++) {
    while (arr[i] < vmin || arr[i] > vmax) {
      if (i_extra == 0) {
        arr_extra =
            curand_log_normal(*random_generator_, n_extra, mean, stddev);
      }
...
int NESTGPU::GetConnectionFloatParam(int64_t *conn_ids, int64_t n_conn,
                                     float *h_param_arr,
                                     std::string param_name) {
  return conn_->getConnectionFloatParam(conn_ids, n_conn, h_param_arr,
                                        param_name);
}

int NESTGPU::GetConnectionIntParam(int64_t *conn_ids, int64_t n_conn,
                                   int *h_param_arr, std::string param_name) {
  return conn_->getConnectionIntParam(conn_ids, n_conn, h_param_arr,
                                      param_name);
}
...
int NESTGPU::SetConnectionIntParamArr(int64_t *conn_ids, int64_t n_conn,
                                      int *h_param_arr,
                                      std::string param_name) {
  return conn_->setConnectionIntParamArr(conn_ids, n_conn, h_param_arr,
                                         param_name);
}

int NESTGPU::SetConnectionIntParam(int64_t *conn_ids, int64_t n_conn, int val,
                                   std::string param_name) {
...
int NESTGPU::GetConnectionStatus(int64_t *conn_ids, int64_t n_conn,
                                 ...
                                 int *syn_group, float *delay, float *weight) {
  return conn_->getConnectionStatus(conn_ids, n_conn, source, target, port,
                                    syn_group, delay, weight);
}
...
int64_t *NESTGPU::GetConnections(inode_t i_source, inode_t n_source,
  ...
  int64_t *conn_ids = conn_->getConnections(i_source_pt, n_source, i_target_pt,
...
int64_t *NESTGPU::GetConnections(NodeSeq source, NodeSeq target, int syn_group,
                                 int64_t *n_conn) {
...
int64_t *NESTGPU::GetConnections(std::vector<inode_t> source,
                                 ...
                                 int64_t *n_conn) {
  return conn_->getConnections(source.data(), source.size(), target.data(),
                               target.size(), syn_group, n_conn);
}
...
int NESTGPU::PushSpikesToNodes(int n_spikes, int *node_id, float *spike_mul) {
  /*
  int *d_node_id;
  float *d_spike_mul;
...
bool isSequence(inode_t) { return true; }

bool isSequence(inode_t *) { return false; }

inode_t firstNodeIndex(inode_t node) { return node; }

inode_t firstNodeIndex(inode_t *) {
  throw ngpu_exception(
      "firsNodeIndex function can be used only with node sequences");
}
